...
## 9. Рішення користувача — 2026-09-09

Чотири з дванадцяти питань §8 закрито; вони гальмували хвилю 1, решта — ні.

| §8 | питання | рішення | що з цього випливає |
|---|---|---|---|
| п.2 | версія релізу | **`3.1.0`, каталог рухається на 3.x** | README веде лише на 3.x; 2.8.x згадується у FAQ, з вітрини знімається |
| п.3 | один шлях чи два | **один: virtual-first із деградацією** | ітеративний ранбук стає режимом деградації всередині virtual-first; правки 1–3 §0 роблять гейти прохідними саме на ньому; хвиля 3 знімає ~20 КБ із чотирьох фаз |
| п.4 | субагент як рецензент | **щабель зняти** | рецензент — тільки зовнішній канал; нема каналу — крок блокується, а не деградує в самоперевірку. `SKILL.md:58` («never a background agent») лишається, `project-intake.md:47-51` і `setup-critic-channel.md:219` вирівнюються під нього |
| п.7 | контракт Критика | **один файл, `data-contract-universal.md` прибрати** | джерело — той, що інжектять скрипти (`assets/data-contract-template.md`); цінне з v2.0 переноситься в нього; посилання `SKILL.md:61` перецілюється; UK-версії контракту не буде |

Відкритими лишаються §8 п.1, 5, 6, 8, 9, 10, 11, 12 — вони не блокують хвилю 1.

**Доказова база рецензії** (огляди A–F, з яких зібрано §3) — `docs/review-2026-09-09/`;
до 09.09 вона лежала лише в скретчпаді сесії.

## 10. Виправлення до самої рецензії

**§0 п.6 і §3.2 казали, що `data-contract-universal.md` несе поле `Origin` — це навпаки.**
Інвентаризація 09.09 (`docs/review-2026-09-09/`, звірка скриптів) показала: `Origin` зі всією
дисципліною SCR-050 лежить у `assets/data-contract-template.md` — тому, який скрипти реально
інжектять, — і на нього вже спирається `phase_2_eq.md:77`. У `universal` його нема взагалі
(жодного входження). Що в `universal` справді було своє — правило «пакет = весь раунд».

Ціна помилки була б видима: за буквою рецензії злиття пішло б у бік, який ВТРАТИВ би `Origin`
і зламав посилання з фази 2. Виправлено при виконанні (`c481e26`).
