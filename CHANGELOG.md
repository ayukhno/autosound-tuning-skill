...
- The number answers *"is this a release?"*, **not** *"how risky is the upgrade?"* When a patch
  ...
  danger in words.
- **Between tags, work lands under `## [Unreleased]`, and a change the user must act on says so under
  `### Breaking`** (hub RELEASE-CHANNEL.md §11, HUB-060). The heading sits above every version and
  is renamed to `## [vX.Y.Z]` when the version is cut; the hub's preflight refuses a patch — or a
  candidate for a patch — whose entry has a Breaking section, because that is not a patch. **Work
  ships in waves** (hub `governance/WAVES.md`): one branch, one PR, the tag on the merged commit. A
  candidate is optional, even for a minor — `scripts/tag-check.sh --candidate vX.Y.Z`, then the tag
  `beta-vX.Y.Z-rcN`, which only `install.sh --channel beta` / `install.ps1 -Channel beta` take; the
  stable channel never sees it. When one exists, the release tag lands on the newest candidate's commit
  plus only the release's bookkeeping — the rename to `## [vX.Y.Z]`, the manifest and the install pins (§11.3).
...
| :--- | :--- | :--- | :---: |
| [v3.0.61](#v3061--2026-09-23--the-installer-upgrades-an-app-it-did-not-install-instead-of-breaking-it) | 2026-09-23 | The installer upgrades an app it did not install, instead of breaking it | — |
| [v3.0.60](#v3060--2026-09-23--w-2-one-form-for-the-intake-one-record-for-a-round-one-line-of-versions-and-variants-as-a-trade-off-front) | 2026-09-23 | W-2: one form for the intake, one record for a round, one line of versions, and variants as a trade-off front | — |
| [v3.0.59](#v3059--2026-09-20--w-1-a-step-reply-is-three-lines-the-language-survives-a-clear-and-a-fact-says-where-it-came-from) | 2026-09-20 | W-1: a step reply is three lines, the language survives a clear, and a fact says where it came from | — |
| [v3.0.58](#v3058--2026-09-18--the-forms-names-are-a-contract-the-installers-bring-the-desk-engine-and-a-wish-is-read-as-a-whole-configuration) | 2026-09-18 | the form's names are a contract, the installers bring the desk engine, and a wish is read as a whole configuration | yes |
| [v3.0.57](#v3057--2026-09-17--the-engine-for-the-windows-guest-on-an-apple-silicon-mac-and-a-release-upload-that-finishes) | 2026-09-17 | the engine for the Windows guest on an Apple Silicon Mac, and a release upload that finishes | — |
| [v3.0.56](#v3056--2026-09-17--phase-1-at-the-desk-on-resonalyzes-engines-the-junction-guards-and-the-tags-own-release-files) | 2026-09-17 | Phase 1 at the desk on Resonalyze's engines, the junction guards, and the tag's own release files | yes |
| [v3.0.55](#v3055--2026-09-17--python3-in-a-new-windows-window-a-doctor-that-says-where-the-key-came-from-and-_n-as-the-series) | 2026-09-17 | python3 in a new Windows window, a doctor that says where the key came from, and _N as the series | yes |
| [v3.0.54](#v3054--2026-09-17--every-rew-reader-names-its-smoothing-joints-read-the-impulse-the-installers-install-only-what-is-asked) | 2026-09-17 | every REW reader names its smoothing, joints read the impulse, the installers install only what is asked | yes |
| [v3.0.53](#v3053--2026-09-16--one-reader-for-measurement-titles-records-that-hold-what-the-method-teaches-one-door-for-the-reviewer) | 2026-09-16 | one reader for measurement titles, records that hold what the method teaches, one door for the reviewer | yes |
| [v3.0.52](#v3052--2026-09-14--the-beta-channel-gets-its-own-copy-of-the-method) | 2026-09-14 | the beta channel gets its own copy of the method | yes |
| [v3.0.51](#v3051--2026-09-13--installer-consistencypy-runs-on-windows) | 2026-09-13 | installer-consistency.py runs on Windows | yes |
| [v3.0.50](#v3050--2026-09-13--a-beta-channel-in-the-installers-and-group-delay-through-the-direct-sound) | 2026-09-13 | a beta channel in the installers, and group delay through the direct sound | yes |
| [v3.0.49](#v3049--2026-09-11--the-reviewer-is-one-channel-with-three-tasks-and-it-names-no-model) | 2026-09-11 | the reviewer is one channel with three tasks, and it names no model | yes |
| [v3.0.48](#v3048--2026-09-11--the-index-no-longer-hides-an-upgrading-note-written-as-a-sentence) | 2026-09-11 | the index no longer hides an Upgrading note written as a sentence | yes |
| [v3.0.47](#v3047--2026-09-09--the-pre-release-review-what-the-documents-promised-is-now-what-the-tools-do) | 2026-09-09 | the pre-release review: what the documents promised is now what the tools do | yes |
...
A patch outside the wave, on the Arbiter's word: urgent (skill #62). Found on the Windows VM while checking
v3.0.60 (TODO S-020).

### Upgrading

- **Nothing to do.** Run the install line again, as always. If an earlier run left "Autosound TCC" unable to
  start with "uv trampoline failed to canonicalize script path", remove the two stale launchers first:
  `Remove-Item "$HOME\.local\bin\autosound-tcc-gui.exe","$HOME\.local\bin\autosound-tcc.exe"`.

### What is in it

- **The TCC upgrade asks uv first** (`install.ps1`, `install.sh`). When TCC's own updater had put the launcher
  there, `uv tool install --upgrade` refused with "Executable already exists", and on the VM the refusal left the
  old app unable to start. Now a tool uv lists is upgraded, and a launcher uv does not own is replaced with
  `--force`, which the installer says it is doing.
- **A running TCC is left as it is on Windows**, with the reason: its files are in use. The person is asked to
  close it and run the line again.
- **The check no longer says "Autosound TCC is not installed" about an app that was there.** It says the app was
  there before and was not upgraded this time.

## [v3.0.60] — 2026-09-23 · W-2: one form for the intake, one record for a round, one line of versions, and variants as a trade-off front

The second wave (`WAVES.md`), shared with tcc's `W-2 · v0.1.43`: the skill tags first, and TCC pins this tag.
The Arbiter's rule for the review (2026-09-22) was to take in everything that was not deferred. The plan is
`docs/PLAN-W-2.md`, and every fork the session decided while he was away is in `docs/W-2-DECISIONS.md`, with its
reason. Eight packages.

### Upgrading

- **Versions can be numbered once per project** (package R, hub #195). A NEW project starts on the
  per-project line: `state/versions/v_NNN.json`, plus `state/slots.json`, which says what each preset holds
  and which is active. An existing project is read as it is. `contract.py check` offers the move
  (`state.py --root <project>/state migrate-line`, then `--apply` on the user's OK). The active slot keeps its
  numbers, the old tree goes to `state/legacy/`, and `legacy-map.json` resolves every old citation. A
  front-end that reads the ledger directly must read both layouts. TCC does so from v0.1.43; an older TCC
  cannot open a project created in the new layout. On the new line the change sheets are
...
- **A — the intake as one served form** (S-033, S-037, S-052; hub #178, #193, #194): `intake_form.py serve`,
  the same page for a terminal session and for TCC's button; 21 questions up front instead of 68, each a choice
  where one exists; the rest asked by the phase that needs it. The target curve and the goal stay on the form,
  and Phase 0 asks only what is still empty. No examples inside a label. The knob rule (S-025), `sources` as
...
- **C — the machine and the installer** (hub #192, S-028, S-049, S-022): git and python3 are run once after
  ...
  rebuilt, or named.
- **D — what a car package carries** (hub #185/#186): the tune's purpose and the control module's state never
...
The first wave run as a wave (`WAVES.md`): the Arbiter walked Phase −1 end to end on a VW Passat B8 /
Helix DSP Ultra S, collected what hurt, and settled the scope at a review. Four packages, one goal —
**a session stops making the person carry the method's internals.** The collection is `docs/TODO.md`
S-024…S-050; the plan it was built from is `docs/PLAN-W-1.md`.

- **A step reply is three lines, and a passed check costs one word** (W-1 package A: S-046, S-038,
  S-040, S-029, S-031 — the umbrella S-041 closes with them; the Arbiter's collection 2026-09-19).
  One complaint seen from five sides: the phase asks the person to carry the method's internals.
  He handed over a whole session reply as «ось приклад зайвої інформації» — what he owed the reader
  was three lines, what arrived was the version match, the clean project file, the ledger HEAD, REW
  being open, four open steps, 44 proposed flaw rows with per-channel counts, a prose reading of the
  midbass arrivals, and then the question. So, in `SKILL.md`'s `✍️ Output Style` and repeated
  wherever a phase text carried the habit (`happy-paths.md`, `phase_-1_intake.md`,
  `phase_0_baseline.md`, `virtual-first.md` 0.6): **where we are** (the whole reconcile folded into
  one clause — *checks: done*), **what is next and what it needs**, **the one question that needs
  him**. A check that FAILED is the only thing that gets sentences, and gets them in the useful form
  — what to do, or the options with the recommended one first. The rest is available on request: a
  tool run to SEE is not a tool run to REPORT.
  - **A verdict is not re-derived in prose** (S-038): after `capture-check`, say what the check
    said and stop. Reading the arrivals is Phase 1's work, where `predict --align` and
    `arrival_triangulate` carry the trust gate and the alias rules a prose reading has to hedge —
    and the same hedged midbass reading came back in a FRESH session, which is what made it the
    text's habit rather than one session's.
  - **Name the thing, not the metaphor** (S-040): the report vocabulary now has one home,
    `naming-and-structure.md` §1a — проєкт · варіант (Phase 1's candidate sets, never
    «конфігурація») · конфігурація = версія `v_NNN` (what stands in the processor, named with its
    slot: `SQ v_007`) · пресет · серія `_N` · фаза. `seed` is «записую перший знімок реєстру», never
    «сію», and nothing is written INTO the processor by the method.
  - **The questioning MECHANISM is never retold** (S-029), and its outcome is never attributed to
    the user: a question that did not reach him is nothing at all, and «вікно питань ви закрили»
    made him correct a report about himself. `feedback-loop.md`.
  - **A plan step carries WHAT it covers** (S-031): `process.py add_step(..., covers=[…])` and
    `add-step --covers a,b,c` take the facts as `open-questions` prints them, and COMPOSE the step's
    name from the first three plus `+N`. `Закрити відкриті поля: project.json (8) і
    dsp_profile.json (5)` was the plan the Arbiter read in his window — thirteen facts, named
    nowhere he could see. The names existed the whole time; the plan dropped them. `open_work` and
    `session-close` print the full list; a window expands it.

- **The reply language survives a `/clear` — `project.json` `language.reply`** (W-1 package B: S-045;
  his evidence 2026-09-20). He cleared the chat at a phase boundary and the session came back in
  English. There was nowhere to read the language: the conversation that settled it was gone,
  `project.json` had no key for it, and `project.language` carried `writes: None`. The next session
  moved the root — it read the front-end's record, NAMED the mismatch, answered in English anyway
  and advised him to go fix it in the app. Now: **three languages, and only one is the method's.**
  - **The reply language** — what the session writes in. `intake.save(<project>, "project.language",
    "<code>")` writes it like any other confirmed answer; `project.py <project> language` says which
    and where from (exit 3 = nobody has answered); `contract.py check` prints it **above the file
    table**, because it decides the language of the very sentence that reports the rest.
  - **The interface language** is a front-end's own — the method never invents it and never stores
    it. A front-end's report of the reply language WINS over the stored value: the app is where he
    set it.
  - **The input language changes neither.** On a Windows VM with no Ukrainian layout he typed
    English while the interface and the reply were Ukrainian; a session that takes its language from
    the last message flips a whole tune on one sentence forced by a missing keyboard.
  A project written before the field still reports the question — `Project.load` merges today's
  skeleton, so `open_questions` names `language.reply` with no migration to run.

- **The mechanisms that carry the rules** (W-1 package D: S-047, S-034, S-035/`#49`, `#47`, S-049's
  first half). Four rules that lived only in prose, or in a comment nobody could act on.
  - **The flaw map ASKS** (S-047). `flaw_map` already knew the question had not been put — it
    stamps every peak it could not check `no positions measured -- staying is ASSUMED, not shown` —
    and wrote that 43 times on a live project while the nine-position set that would settle it sat
    on the tuner's disk. His question: «чому скіл сам не запитав про такий замір?». A run with
    assumed rows now ends with the REQUEST: the channels, `<ch> p1…p9_<N> (sw)`, and the
    `capture-start` line. **The phase does not close over them** — `enter-phase 1` refuses until
    the round was opened or the Arbiter's answer is recorded as a decision. Only a PEAK's verdict
    turns on the microphone moving; a `cabin_null` or a `non_min_phase` row asks for nothing.
    Phase 0 also asks one sentence earlier: are there position measurements at all?
  - **The feedback rail can COMMENT** (S-034): `post_comment(<issue url or number>, body_file,
    channel=…)` beside `post_feedback` — same closed `CHANNELS`, same `guarded_run`, same verified
    URL, plus the issue number, so a comment that landed on another thread is refused. «Add this to
    #39» had no guarded path at all, and the way round it was a raw `gh` call with the target
    chosen by whoever was typing — the exact shape the gate exists to refuse.
  - **And its title says what the finding is about** (`#49` / S-035). It was built as
    `Feedback: <car> · <DSP>` and took no argument, so every finding from one car arrived under one
    name — and the 24-hour duplicate guard keys on the title, so the SECOND genuine finding of a
    session was silently skipped as a duplicate of the first. The title now comes from the caller,
    or from the body's first heading, with `car · dsp` kept as the suffix.
  - **`naming.py check` stops saying plain `ok` for a title it normalised** (`#47`). The comparison
    runs on `name_key`, so `sw_01 (sw)` and `sw_1 (sw)` are the same measurement — right, and
    wanted. The silence was the bug: `check` said `ok` on all 16 while the front-end, which finds a
    measurement by its literal REW title, said `0 usable, 16 missing` over the same session, and
    sixteen good measurements were registered as unusable. `validate_series` now returns `matched`
    and `renames` (`{title on disk: canonical title}` — the "new name" column), the CLI prints
    `ok*` with the rename, and `contract.py check` names it too. The canonical form is what
    `generate_name` emits, and the fix is a RENAME: REW's `uuid` survives one.
  - **The doctor names the desk engine** (S-049, first half). Present or absent, the pin, the
    platform, and the command that fetches it — `engine_status()`, which builds nothing, because a
    doctor that builds changes the machine it reports on. The Arbiter reached step 1.3 of a live
    tune before learning his MacBook had no engine, and `install.sh`'s own comment gives fetching's
    reason as a person «would otherwise discover that in the middle of a tune».

- **Provenance: a fact whose origin is not written reads as everybody's word** (W-1 package C:
  S-024, S-036 / `#48`, S-048). One defect seen three times in one wave, and in all three the data
  was fine and the label was missing — which is worse than missing data, because nobody goes
  looking.
  - **An imported fact says it is imported** (S-024). Seven `channels[].fs_hz` came across in an
    export still carrying `"source": "measured"` with the source build's own timestamp, and landed
    in a project created days later on another machine looking exactly like measurements taken
    there. `project.py <dir> mark-imported channels.tw-L.fs_hz --from <other project>` flips the
    ORIGIN and nothing else: the value stands, `at` keeps the moment it was measured THERE,
    `source` keeps saying `measured` — because it was. **No second measurement is owed** (the
    Arbiter, 2026-09-19: «імпеданс складна штука і міряти його другий раз це подвиг»), a protective
    high-pass may be derived from an imported Fs, and `contract.py check` reports the carried-in
    facts — and gates on none of them, which is now asserted rather than left to the next editor.
  - **The protective record carries WHO wrote it** (S-036, `#48`): `user | front_end | default`.
    Ten channels marked `OFF` in one second — rears included, which were not in the series — is a
    default being read downstream as ten answers, and the measurements themselves showed the 100 Hz
    LR24 the record denied. `should_de_embed` now answers `check` for a bulk `default` instead of
    "measured unfiltered"; `protective.bulk_default()` names the whole signature (every channel
    OFF, nobody's word, channels outside the round) and the exporter prints whose word each line
    is. `set_protective` also refuses a channel the round never captured.
  - **A closed round can be corrected, visibly** (`#48`): `capture-protective --amend <cap_id>
    --reason "…" <ch> …`. The round most likely to need a correction is the one an exporter has
    already read, and the only way to say so was to open a NEW round on the same version and close
    it saying nothing was measured in it — which makes "capture round" mean two things. The
    amendment writes no state: it is a `capture_protective` event carrying the round's id, so the
    replay picks it up as the last word, with `amends` and the required reason on it.
  - **A foreign series number carries its origin** (S-048). `_N` numbers the series of ONE project;
    two projects both have a `_49`. A session said out loud that a set was «серія `_49` зі старого
    проєкту» and then opened the round as series 49. A round now refuses a number that is not this
    project's own — already used, or the next after its highest — unless `--origin
    <project>:<their _N>` says where it came from.

- **NOT part of W-1, and shipped on the branch rather than pulled back out: the intake as a FORM
  the skill serves itself — `rew_tool/intake_form.py`** (S-033; S-033/S-037 were deferred at the
  wave's review, and eight of the Arbiter's points against its first page are still open). It is on
  disk, it is not the intake route, and no front-end should build against it yet. `intake.py` made the questions data; this is the page that renders them and
  the loopback server that writes the answers back through the method's own writers. It lives in the
  skill, not in a front-end, because the questions must exist once: a window carrying its own copy
  of them drifts the first time a field is added, and a terminal session has no window at all —
  `serve` gives it one. All 71 fields on one page: groups as tabs, each couple as ONE control (the
  seat included, which is the answer that once had to be voided), the per-channel and per-amplifier
  halves as tables rather than 12 × N controls, and every field coloured by what is owed — red
  required-and-missing, yellow optional-and-missing, green answered, grey lands in prose and a
  recorded decision, so the form says so instead of pretending to read it. The three probes
  (`rew.api_reachable`, `rew.input_clip_checked`, `dsp.readable`) are marked, not asked: a form that
  asks them asks the person to do the tool's job. The page decides nothing — it prints
  `contract.py check --gate`'s verdict — reaches no network (no CDN, no font), and refuses to serve a
  project directory that is not there. **Labels are data**: a session translates the method's English
  as it speaks, a static page cannot, so `intake_i18n/<lang>.json` carries them per field id, falling
  back to the English in `FIELDS`; Ukrainian is the one that exists today, the rest come before the
  release. `intake_form.py serve|render|state`, `--selftest`.

- **The intake's fields, enumerations and couples ship as data — `rew_tool/intake.py`** (hub SCR-059,
  from TCC). Phase −1 was already a fixed set of questions with fixed answers: the DSP half says so
  out loud (`dsp_profile.CAPABILITY_CHECKLIST`), and the rest — the car's four parts, the channel
  map, the measurement chain, the rig, the reference seat, the curve seed — existed only as prose
  and as a conversation. Watched twice on the Arbiter's own machine, on two cars and two processors:
  no window asked the car half at all, it arrived in long free-text messages **after** the Phase-0
  gate refused (project open, interview over), and the reference seat, asked as two turns, was
  answered "driver only", corrected five minutes later and a recorded decision had to be voided.
  Now `intake.py fields` lists every field with its id, group, the question in the method's own
  words, whether it is required, its enumeration, and **where the answer lands**; `intake.py
  couplings` names the questions that are ONE control and why (the four parts of the car, the seat,
  the purpose and its formats, a slot and its tier, the two inputs); `intake.py gate` prints the
  files and keys `contract.py check --gate` decides on — read off `contract.GATE_REQUIRED` itself,
  so a front-end can ask for the missing half **before** anything opens; `intake.py missing <project>`
  splits what is answered from what is open, and reports the goals, taste and curve seed as *not
  machine-readable* rather than as gaps, because they live in prose and in recorded decisions.

- **A capture round says which counter its version is, and a `v_NNN` nobody banked is refused**
  (hub `TCC-022`, from TCC; the Arbiter's stopper for the current wave). On a project made by TCC's
  Copy car — which carries `project.json` and the DSP profile and deliberately no `state/` —
  `capture-start` opened **four rounds in a row at `v_001`**, a version that comes into existence
  only when the first snapshot is banked, and said nothing; meanwhile `apply.propose` could not
  produce a settings sheet and also said nothing («why can't I call up the Load form?»). One half
  lenient, the other mute, both about the same missing fact.
  - **The answer, stated:** the ledger is a precondition of a LEDGER-BOUND round, not of the
    capture flow. A Phase-0 baseline is measured before anything is banked, opens at `_1`, and now
    records `version_kind: "series"` — the round saying it is not ledger-bound, instead of leaving
    a reader to guess from the spelling. A `v_NNN` with no snapshot on disk is refused, and the
    refusal carries both ways on: bank the state (`apply.propose`), or open the round at its series
    number.
  - **An unplanned SKIP is marked the way an unplanned arrival is** — `planned` on the round's
    `skipped` entry and on `capture_skipped`. `expected[]` is not a closed set, so a reader cannot
    assume everything in `skipped` was ever asked for.
  - **And it no longer disappears:** `contract.py`'s `round_verdict` intersected `skipped` with
    `missing`, and `missing` comes from `expected` — so a skip of a title the round never expected
    was neither taken, nor missing, nor listed as skipped. Four rear titles went that way. Every
    skip is now reported, with `skipped_unplanned` naming the ones nobody asked for.

- **A writer for the half that had none** (the same ticket). `dsp_profile.set_field` already took
  the processor's answers; the car, the channel map and the measurement chain had no route in that
  did not go through a conversation. `intake.py set-car | set-channel | set-amp | set` write through
  `Project` — and refuse what the questions themselves imply: **the car is four parts**, so a blank
  generation or body stops the write (a build with no body recorded matches no cabin and no earlier
  build on this car), and **a slot needs its tier in the same breath**, since slot letters repeat
  across tiers and one letter is a legal address in more than one of them (SCR-042). A value outside
  its enumeration is refused naming the choices, a field that belongs to another writer is refused
  naming which one, and **nothing is written when anything is refused**. New `project.json` keys —
  `source.{kind,connection,listening_input,measurement_input}`, `measurement.{loopback,sample_rate_hz,interface}`,
  `channels[].{position,enclosure,condition}`, `mic.calibration_file_90` — are in `project-schema.md`.

- **The method says out loud that a front-end may collect Phase −1 up front** (`phase_-1_intake.md`
  §0.6, the ticket's last ask). Collecting is supported; what it still owes is unchanged and now
  written down: the phase is opened first (`enter_phase("-1")` is the entry condition, not a step),
  every ruling is still recorded, the gates do not move (names written before any measurement,
  install verified before any tuning, the curve seeded with the person and never defaulted), and the
  prose the human and the Critic read is still written. `contract.GATE_REQUIRED` and
  `dsp_profile.PLAUSIBLE_RATES_HZ` are public for the same reason the form's names are: a consumer
  reads them, and a copy over there would drift from the check that enforces them here.

## [v3.0.58] — 2026-09-18 · the form's names are a contract, the installers bring the desk engine, and a wish is read as a whole configuration

> **Upgrading — an install now brings Phase 1's desk engine on a machine that has no .NET SDK.**
> ~30 MB from the tag's own release, checked against its `SHA256SUMS`; a machine with the SDK is
> unaffected, and the engine still builds there on first use. `--no-engine` / `-NoEngine` refuses
> the fetch, `--engine` / `-Engine` takes it even where the SDK could build one. A platform or an
> engine pin the release carries no archive for is said out loud, not treated as a failure — the
> SDK route stands. `--uninstall` now removes the engine folder with the rest.
>
> **And `resonalyze_engine.py run` does one more engine pass per wish.** A wish is now also run as its
> own whole configuration, so a run with two wishes takes two engine runs more than it did, and the
> report and `variants.json` carry a new section and a new key (`full_variants`, with `chain` and
> `best_junctions` beside it). Nothing existing changed shape; a caller that reads the wishes' probe
> numbers reads them exactly as before.

- **A wish is read as a WHOLE CONFIGURATION, not only at its junction (issue #38, the user's decision
  2026-09-18).** Until now a wish with a corner was a probe — that one junction, each side after its
  own delay — and the rest of the configuration held still. But a crossover moved at one junction
  moves what every other junction is summing, and the tuner hears the whole chain. So every
  computable wish now gets a third pass of its own: what stands after the repairs is GIVEN rather
  than searched, the wish's edges go in at its own junction and nowhere else, **Auto delay runs again
  over the whole chain**, and every junction is then read as it stands (`"probe": []` in the layout is
  a reading with no variant — the probe's first entry has always been the configuration itself). Each
  variant says its own junction's sum loss, what the other junctions gained or lost, the delays that
  moved, any polarity that flipped, the limits checked again on its settings, and a refusal when its
  delays no longer fit the device. **What it found on the Passat** (`ir-v7_49`, 2026-09-18): "BE4
  between tweeter and mid" gains 0.16 dB left / 0.60 dB right of sum loss at its junction and moves
  eight delays while **flipping both tweeters' polarity**; "BW2 between sub and midbass" costs
  0.41 / 0.32 dB over the chain and moves the whole chain by about 3.9 ms, the sub's own delay from
  0.00 to 1.03 ms. The probe's DIRECTION agreed with the configuration's on both wishes; its magnitude
  is a different quantity (a junction ranking score, not a sum loss), and the delays and the polarity
  are what a probe cannot show at all. Held by the synthetic `smoke` in CI and by the module's offline
  selftest; a golden would mean teaching `acceptance` a sequence of three runs instead of one, and that
  waits for a second car to need it.

- **The phase documents say what Phase 1 does with NO engine.** It was only implied — "the per-driver
  candidates stay beside it" — and implied is not a path a session can follow. `virtual-first.md` 1.3 now
  says it outright: the engine is optional, nothing turns it on, `--no-engine` / `-NoEngine` refuses it at
  install, and with none the tool says so and stops rather than falling back silently. Phase 1 then runs
  the per-driver way it ran until 2026-09-17 — `xover_candidates` → `xover_select` with every corner
  checked, `predict --align` joint by joint with the RES-013/RES-016 guard, levels, sums, EQ, scene
  presets — and what is genuinely unavailable is named: the whole configuration at once, Auto delay's
  staging and gain balance, and the price of a wish. `capabilities.md` carries the same in its row.

- **The installers fetch the desk engine (`docs/TODO.md` S-020, `DESIGN-2026-09-17-phase1-variants.md`
  §5c).** Since `v3.0.57` a tag's release carries `resonalyze-engine-<pin>-<rid>.zip` for `win-x64`,
  `win-arm64` and `osx-arm64` (hub `RELEASE-CHANNEL.md` §12) and nothing knew they were there: the
  engine was a `install-binary --from <zip>` by hand, or the .NET SDK. `resonalyze_engine.py
  fetch-binary --tag vX.Y.Z` now computes the file's name from `ENGINE_PIN` and `rid()` — that the
  name is computable is what §12 requires it for, so no installer lists a release or picks an asset —
  takes it from that tag, checks it against `SHA256SUMS` and installs it where `engine_command`
  already looks. **A file that arrives and does not match is refused with nothing installed** (exit 3);
  **a release that carries none for this pin and platform is an answer, not a failure** (exit 4), so a
  caller says which way it went instead of falling through in silence. All three installers carry the
  same decision — fetch only where there is no SDK — and `scripts/installer-consistency.py` (check 5c)
  holds them to it. Verified on this Mac: the fetched engine ran the whole synthetic set
  (`resonalyze_engine.py smoke`) with nothing built; the installer's own step is run on Windows when
  the VM next takes a tag.

- **The form's `FORM_*` names are a consumer surface, and the selftest says so** (hub SCR-057,
  from TCC). TCC's window stopped keeping its own copy of the Arbiter's form in its `v0.1.41`:
  `core/form_report.py` reads the address, the five question ids, the choice words, the person's
  kinds and the "counts as sent" rule out of `rew_tool/gates/side_effect.py`. Two copies of one
  form is how a question id moves on one side and a column in the sheet is quietly filled by the
  other, so one copy is right — but it makes those names something another repo depends on, and
  the cost of a rename is invisible from here: TCC's `is_available()` finds a name missing,
  `post_url()` comes back empty, and the form route disappears from its window with GitHub left as
  the only route offered. No error, no exception, nobody told. The gate's selftest now pins the
  surface — every exported name, the five entry ids, the keys of `FORM_KINDS`, `FORM_IMPACTS` and
  `FORM_PERSON_KINDS`, and the `(ok, detail)` pair `verify_form_reply` answers with — so the break
  happens in our own suite first. **When any of it moves, or when the form's own questions change,
  the move is named in the Upgrading note of the tag that carries it**: TCC labels the choices by
  KEY in four languages, and a key it has no label for is dropped from the window rather than
  shown, so the note is what tells its side to add the label. Nothing in the method's behaviour
  changes.
- **`FORM_LABELS` — the form's question names as data** (the same ticket's third ask). A report the
  form did not take goes to the clipboard instead, and its lines are read beside the sheet, whose
  columns wear exactly those words; TCC wrote them from its own copy, which was the last place the
  sheet's columns existed twice. They are exported keyed by the question id and in the form's own
  order, so the dict `form_answers()` returns can be labelled without a second table anywhere — and
  the selftest asks that every answer the form takes has a label and no label names a question that
  is not asked, against `form_answers` rather than against a copy of the ids. The words are the
  form's Ukrainian, not a translation: TCC keeps its four languages for what a person CHOOSES, while
  a column's name belongs to the form.

## [v3.0.57] — 2026-09-17 · the engine for the Windows guest on an Apple Silicon Mac, and a release upload that finishes

- **A third engine build: `win-arm64`** — a Windows guest in Parallels or UTM on an Apple Silicon Mac is ARM, and the
  wrapper computes `win-arm64` there, so that machine found no prebuilt engine at all (or ran the x64 one through
  Windows' emulation). Built AND run through the synthetic set on a `windows-11-arm` runner, like the other two.
- **The release upload clears a stuck asset record and retries, file by file** (`engine-binaries.yml`): measured on
  `v3.0.56` — an upload that fails leaves its asset behind in state `starter`, holding the name, and `--clobber`
  (which deletes by name through the assets list) does not clear that one, so every later attempt answers
  `HTTP 500: Error saving asset`, on the run and on any re-run. The step now deletes such a record by ID before each
  of three attempts per file, and prints what the release holds at the end. `v3.0.56` itself has no engine archives:
  its run carries the old step, and only a tag push can attach them (hub #173).

## [v3.0.56] — 2026-09-17 · Phase 1 at the desk on Resonalyze's engines, the junction guards, and the tag's own release files

> **Upgrading — what a person may notice.**
> 1. **Phase 1's crossovers and delays come from Resonalyze's engines** through a wrapper in the skill's tree
>    (`engines/resonalyze`, the fork as the submodule `vendor/Resonalyze`). Running them needs either the .NET SDK
>    (`~/.dotnet` is enough) or a prebuilt engine for your platform (`resonalyze_engine.py install-binary --from <zip>`;
>    the builds come from CI). Nothing else in the skill needs .NET.
> 2. **EQ is in two parts:** `eq_propose --part 1` (the coarse per-driver resonances) is Phase 1's and is banked BEFORE
>    the delays; `--part 2` (the pairs' L/R shape, then tone) is Phase 2's. Phase 2's order changed: pairs per band →
>    junctions per side → sub with mids → each side whole → the scene's two presets by ear → everything together →
>    tone, then the centre, then the rear.
> 3. **The scene is centred by level in Phase 1** (the base) and the time preset is compared by ear in Phase 2c; the
>    listening cheat-sheet gained the protocol in four languages.
> 4. **No GitHub account is needed to send a finding:** a session sends it to the Arbiter's form instead, after asking
>    who is writing and showing the text.
> 5. **Above 1 kHz a mid↔tweeter delay is confirmed on a measurement of the pair.** The joint is read twice — the whole
>    record and the direct sound — and where the two disagree, or the proposal sits a cycle from the direct-sound
>    reading, the desk no longer picks: the joint comes back UNVERIFIED with both candidates, and one sweep of the two
>    drivers together settles it. Below 1 kHz nothing changed.
> 6. **A crossover candidate is also judged by how much its group delay swings across the junction** — against the only
>    published audibility threshold (Blauert & Laws), clamped below 500 Hz. Where Phase 1 ranks candidates you now see
>    **two leaders**: Resonalyze's own and the alternative by this experimental term; the run keeps the engine's.

- **Resonalyze's engines behind a wrapper** (design `docs/DESIGN-2026-09-17-phase1-variants.md`; the fork session's brief
  and goldens, hub #159): `rew_tool/resonalyze_engine.py run` builds the layout from the project — driver types from the
  channel map, never the engine's suggestion; each solo's protective high-pass divided out from the set's manifest; the
  device profile's delay range, two tiers where the Helix's add — runs Auto crossover, re-searches every front-chain
  junction whose edge broke a limit inside the limits, runs Auto delay, and reads the tuner's wishes (`--wishes`) against
  the best. Four goldens on the Passat's `ir-v7_49`, `acceptance` 4 of 4, byte-identical to the fork's on one machine.
- **The tuner's wishes in free words** (`rew_tool/xover_wishes.py`): "BE4 between tweeter and mid, BW2 sub-midbass, not sure
  between midbass and mid", in English or Ukrainian, each checked against the device's families and slopes and the fragile
  driver's Fs floor; a wish that breaks a limit is not computed and the nearest allowed setting is named.
- **`eq_propose --part 1|2|all`** with `recheck_junctions` on every package: the junctions its bands reach into, so the delay
  there is re-read before banking.
- **`rew_tool/sums_export.py`**: the predicted sums (ALL, L, R, ALL+C) as visualizer files beside the target, smoothed 1/6
  (or 1/3 … 1/48, REW's psychoacoustic, none), the variant's label in each.
- **`rew_tool/scene_presets.py`** (research RES-011, hub #161): from the base after 2c, one time preset per rung
  (0.15–0.30 ms, the near side later, its cut reduced 16 dB/ms, both channels trimmed to the base's centred level), each
  described by its pull with the time and level parts apart; deltas for `apply.propose`.
- **`scripts/upstream-drift.py`** reads the wrapper's `//` headers: 25 Resonalyze files watched at the pin.
- **CI:** an `engine` job builds the pin and runs a synthetic set end to end; `engine-binaries.yml` builds the
  self-contained engine for win-x64 and osx-arm64 — each on its own runner, each run through the synthetic set there —
  and **a tag push attaches them to that tag's release** (hub HUB-070, `RELEASE-CHANNEL.md` §12): draft → files →
  publish, a pre-release that is never `Latest`, `SHA256SUMS` beside `resonalyze-engine-<pin>-<rid>.zip` and the fork's
  licence and third-party notices inside each; a release already published for the tag is left alone and the job says
  so. Another platform builds from the .NET SDK on demand.
- **`predict --align` does not overrule a tune above 1 kHz** (research RES-013, hub #168): a whole-cycle
  disagreement between the sum-loss score and the arrival witness is recorded, not resolved — `chosen:
  "unverified"`, both candidates with their sum loss, the witness read through the full record, the 6-cycle cut
  and a 2-cycle cut (unconfirmed), the untuned pair's arrival beside them, and the joint named NOT BANKABLE in the
  warnings and in the proposal; the chain keeps the score's best only so the joints above can still be read.
  Measured: on the BMW F30 archive the full-record witness put five owner hand tunes 4.2–4.9 cycles off on every
  right side, where the old code would have proposed an answer 4–5 cycles away. Below 1 kHz unchanged. **Amended the same day by research RES-016 (hub #172):** RES-013's trigger watched the score and the full-record witness
  disagree, and on 3 of the 10 BMW cells they agreed and were both ~4 cycles wrong; the guard now reads the pair a second
  time through a 2-cycle direct-sound cut and marks the junction when the proposal sits ≥ 0.75 cycles from that reading,
  when the two witnesses sit ≥ 0.5 cycles apart, or on RES-013's own disagreement — with no cut, when the proposal moves
  the junction a whole cycle from the delay the DSP holds. The cut is a guard, never an answer. On the archive: 6 of 10
  marked, every cell 0.85–4.5 cycles from the owner's tune, none of the four within 0.15.
- **The junction's group-delay swing joins the crossover ranking** (research RES-014, hub #170): `dsp_math.junction_gd_swing`
  — the 95th − 5th percentile of the summed crossover pair's group delay over fc/2…2·fc, bulk excluded — against the
  Blauert & Laws threshold (`gd_threshold_ms`: 3.2 ms at 500 Hz … 2 at 8 kHz on log frequency, **clamped to 3.2 ms below
  500 Hz**, a decision named in code and report). Reported in `predict`'s junction rows and ladder, under each
  `analyze-joints --from-state` row; scored at 1 dB per ms over the threshold in `xover_select.select_neighbor_pair`
  (with the electrical filters given; `best_without_gd` / `best_under_threshold` beside the pick) and in
  `resonalyze_engine.rerank_pool`, which re-ranks the engine's exported top-5 pool and shows **two leaders**. Measured on
  the Passat's woofer↔mid: the winner moves by 5.37 dB of margin, the swing falls from 7.0/4.9 to 3.3/2.1 ms; silent at
  mid↔tweeter. A ripple term is NOT added (S-006 answered in the negative).
- **Without GitHub, a report reaches the Arbiter's form** (hub TCC-017): `gates/side_effect.post_feedback(…, via="auto"|"form")`
  sends the same text to his Google Form when `gh` is missing or not signed in — who is writing, the kind, for a problem
  how far it stops the tuning, the message in the person's language — and counts it sent only when the form confirms.
  Text only; pictures still need GitHub. TCC's window sends the same form.

## [v3.0.55] — 2026-09-17 · python3 in a new Windows window, a doctor that says where the key came from, and _N as the series

> **Upgrading — what a person may notice.**
> 1. **Windows: a new window runs the installer's Python.** The installer now puts `~\.local\bin` at the
>    front of your user PATH. Before, a new PowerShell window ran the Microsoft Store's `python3`
>    shortcut, and the method's tools did not start there.
> 2. **`doctor` says more.** It names where the key and a forced CLI came from — a config file, or the
>    environment every program inherits — and what answered its live call. A `GEMINI_BIN=gemini` left from
>    an old setup while `agy` is installed is now a ✗: remove it.
> 3. **The key belongs in the reviewer's config file** (`~/.config/autosound/critic-env`,
>    `%APPDATA%\autosound\critic-env` on Windows), not in a project, an environment variable or a shell
>    profile. With a key present the API is asked first, and a model the key cannot call stops the round
>    with the key's list.

- **Windows: `python3` in a new window is the installer's Python** (S-016). `install.ps1` moves
  `~\.local\bin` ahead of the WindowsApps folder in the user PATH, through the registry so the value keeps
  its kind, and its check reads the PATH a new window gets instead of the one it rebuilt for itself — which
  is how the Store's shortcut hid behind a working install.
- **The plan no longer promises to install an app that is there** (S-009): the app's launchers in
  `~\.local\bin` make it "updates to its newest release".
- **`doctor` says where things came from, and what answered** (S-015): the origin of the key and of a forced
  CLI; a `GEMINI_BIN` forcing the closed `gemini` while `agy` is on PATH; this platform's config path in
  every hint; a live call that walks the round's own ladder (an API failure hands over to the CLI, a model
  the key cannot call stops at the choice) and a mode line that says what answered, or that nothing did.
- **`_N` numbers the measurement series, not the DSP state** (the user's correction, 2026-09-17): a DSP
  state has several series, and an impedance measurement is tied to none. The grammar's docs, the refusals
  and the docstrings say so; titles parse exactly as before.
- **README and FAQ (English) say what the method does now:** the install options and their form on each
  platform, the sign-ins at the end of the install, `gh` only with `--github`, the reviewer through `agy`
  with a named model and `doctor` as the check, the key in the machine's config file, measurement titles,
  `capabilities.py find` — and the EQ is loaded into the DSP, not retyped (S-007). The uk/de/pl
  translations say they lag.
- **The Windows VM run of v3.0.54** (S-012): the update took, the method's checkout stayed clean, `gh` and
  `omp` come only when asked, and the reviewer answered through `agy` with the prompt on stdin (hub
  TCC-014). S-010's window was not seen.
- **Windows, on the VM (2026-09-17):** the branch's installer moved `~\.local\bin` ahead of WindowsApps and a
  new window then ran Python 3.12.14; the plan named the installed app; `doctor` named the key's file and
  said the API answered.

## [v3.0.54] — 2026-09-17 · every REW reader names its smoothing, joints read the impulse, the installers install only what is asked

> **Upgrading — three things a person may notice.**
> 1. **Joint numbers change.** `analyze-joints` reads the solos from their impulses through six cycles
>    (`--fdw N` or `--gate MS`), not from REW's frequency response. A solo off the loopback base is
>    refused; a junction with an RTA pair is read steady and says so. On the reference car one junction's
>    polarity flipped and one delay moved 0.8 ms — re-run a joint table made with v3.0.53 before entering
>    its numbers.
> 2. **No reading follows REW's view smoothing any more.** `verify` and `analyze-batch` read 1/6, the EQ
>    cuts of `rew_tool.py` read `Var`, `ear_suspects` and `verify_prediction` read 1/48, `spot_check` 1/6
>    unless `--smoothing` says otherwise. A number that matched the view may differ now; the view itself
>    is never changed. `rew_api.set_smoothing` is gone — ask on the read, `get_fr(mid, smoothing=…)`.
> 3. **The installers install `gh` and `omp` only when asked** — `--github` / `-GitHub`, `--with-omp` /
>    `-WithOmp` — and ask no question on the way. What a machine already has stays.
>
> Nothing on disk needs migrating.

- **Every REW reader names the smoothing it reads** (TODO S-013, hub #151, hub TCC-015). A session had
  read a curve by switching the Arbiter's smoothing off and back; REW computes any smoothing on the read
  (`?smoothing=`), and the measurement's own setting stays as it was — verified on REW 5.40 beta 132.
  Which smoothing each analysis asks for was researched on the live car, in REW's documentation, in
  papers and in practice (`docs/RESEARCH-2026-09-17-reader-smoothing.md`) and decided by the user:
  1/6 for tone and level, `Var` for EQ cuts, 1/48 as the finest read — **`None` and `1/48` are one
  level** (`rew_api.FINEST_SMOOTHING`): above 640 Hz what was finer than 1/48 differed 81–88 % between
  nine head positions, and below 320 Hz the two are the same curve. REW's own group delay in the
  interactive report reads 1/12, the width of the gated reading beside it. `curve_view` accepts 1/48 as
  clean. `rew_api.py --selftest` reads every `get_fr` / `get_group_delay` call from the source and holds
  each to its decided ask, so a new reader cannot follow the view unseen.
- **`analyze-joints` reads the impulse through a window** — six cycles, the window `predict` reads
  junctions through (`windows.py`, hub RES-006), by `predict._read_pair`'s rules: a gate under five
  cycles at `fc` or arrivals further apart than the window read steady, and the row says why. REW smooths
  phase with magnitude; a window leaves the late sound out instead of averaging it in. The measured pair
  goes through the same window.
- **`flaw_map.py --rew N`** (#35): the flaw map from the solos a Phase-0 round left in REW, with what was
  in each chain taken from the round's protective record — the tool `phase_0_baseline.md` names no longer
  needs Resonalyze's v7 files.
- **The installers ask nothing and install only what is asked** (#25): `gh` only with `--github`, `omp`
  only with `--with-omp` (the plan screen names both flags when they would add something);
  `installer-consistency.py` checks both defaults in both installers. CI parses `install.sh` with
  `bash -n` and `install.ps1` with PowerShell's parser.
- **Phase −1's procedure lives in its phase file.** `phases/phase_-1_intake.md` carries the runbook (the
  first start, both interviews, generating the project files); `core/project-intake.md` keeps the doctrine
  under its old section numbers, so every pointer into it still lands.
- **`installation.md` tells one install story** — the installer, and a developer clone; the plugin
  catalogue stays pinned at 2.8.3 until 3.1.0 and is no longer the recommended path.
- **`docs-check` rule 8, `protective-floor`:** every protective high-pass floor written in the documents
  (1.1 × Fs, ≥ 24 dB/oct — twelve copies in eight files) must equal the pre-sweep gate's constants.
- **`capabilities.py find "<words>"`** prints the rows of the capabilities board that match, English or
  Ukrainian, best first — three rows instead of 41 KB.
- **Three contradictions in the documents closed** (the reviewer's model classes pointer in SKILL.md,
  phase 2's checkpoint labels, the +6 dB boost ceiling now stated where SKILL.md cites it).
- **Windows:** `install.ps1`'s `gh` and `omp` defaults are parsed by CI and have not been run on a VM
  (TODO S-012).

## [v3.0.53] — 2026-09-16 · one reader for measurement titles, records that hold what the method teaches, one door for the reviewer

> **Upgrading — three things a person may have to do.**
> 1. **The reviewer is one command:** `python3 scripts/autosound_ai.py critic|advisor|ask <package>`,
>    and `doctor` to check the channel. The shell wrappers (`scripts/{gemini,claude,codex}_{critic,advisor}.sh`,
>    `gemini_{critic,advisor}.cmd`) are gone — a script or a habit that calls one gets "no such file".
>    `GEMINI_FALLBACK_MODEL` is no longer read: a dry quota is a refusal that names the ladder, not a
>    weaker model swapped in.
> 2. **A failed reviewer call exits 4**, lists its reasons and files nothing as a review; it used to exit
>    0 and save the outgoing prompt as a "review". Inside an agent session a CLI reviewer is not started
>    (exit 4) — a key works there, or `AUTOSOUND_ALLOW_NESTED_CLI=1` on purpose. A front-end reading
>    the exit code: hub SKL-037.
> 3. **A project seeded from another one marks the facts it copied as inherited, and the pre-sweep gate
>    refuses a fragile driver's inherited Fs until the Arbiter decides** — confirm the value
>    (`project.py <dir> set-channel <code> fs_hz=<Hz> --source user`) or measure it here (`<code> (imp)`).
>    A project started from scratch, or seeded before this version, carries no marks and is not affected.
>
> Nothing on disk needs migrating: every title that parsed before parses to the same parts, and the new
> flaw and fact fields are additive.

- **One reader for measurement titles** (#39: #33, #34, hub #152). `naming.parse_name` is the only one;
  `state/process.py` read `_N` with a pattern of its own and lost it on `c_49 (sw) x0`. A free-form
  clarification after the method is part of the title — **another measurement in the same series**
  (`r-L_17 (sw) noXO` is `_17`, and not `r-L_17 (sw)`) — kept as `params`; a position in it is still a
  position. `(imp)` is a third method with no `_N`. `explain_name` says why a title is refused, and
  `generate_name` refuses a ledger version (`v_001`) for `_N`. `naming-and-structure.md` §3 defines `_N`
  as the number of the DSP state measured — not the ledger's `v_NNN`, not moved by saving — and writes
  down that capture titles are matched as typed: a mismatch is a question, never a guess.
- **A capture the check does not apply to holds no step** (#29). `check_captures` keeps `applicable`, an
  `(rta)` is not "unusable", the journal lists it under `not_applicable`, and the counts and the session
  header say "not checked (not a sweep)".
- **A skipped capture is reported with its reason** (#32): `contract.py check` shows "16/20 captured,
  4 skipped" with one line per skip, MISSING only for what is neither, and no more `vv_001`.
- **A fact carried in from another project says so** (#36). Facts take `origin: inherited` and
  `inherited_from`; `project_seed.py` marks what it copies (a bare channel `fs_hz` is wrapped) and writes
  `seeded_from`; `contract.py check` lists inherited facts and source paths that no longer exist
  (reported, never gated); the pre-sweep gate reads the Fs as the fact.
- **The flaw map holds a level tilt and a distortion percentage** (#30, #31): kind `level_tilt` (the
  pair, `level_db` first minus second, `bw_oct` the band) with action `level`; `thd_spike` rows carry
  `thd_pct` and `fundamental_db`, `level_db` null, printed as "THD 2.6 %"; the contract names older THD
  rows owing a percentage.
- **One reviewer door** (#41, #28, hub #150). `agy` gets the prompt on stdin as a stream-json message —
  it had been handed a file path to read, which headless mode denies on a clean Windows machine. The
  clipboard rung writes the PACKAGE into the project as `process/reviews/<ts>-<role>-package.md` (never
  into the method's checkout) and `--mode clipboard` asks for it by name. `AUTOSOUND_CRITIC_CLI_ARGS`
  passes per-run flags. `doctor` carries what the wrappers checked — the agy→gemini symlink, macOS
  quarantine, the closed gemini sign-in, the key's shape — and a live smoke on the named model.
  `setup-critic-channel.md` says where the key goes on each platform and that `agy` signs in rather
  than taking one.
- **Updating the method no longer says the update did not take when it did** (Windows VM,
  2026-09-14, installing v3.0.52). Both installers compared HEAD with `FETCH_HEAD`, which for an
  annotated tag — every release — is the tag object, not its commit, so every update of an installed
  method warned. They now compare with the commit. A tag is also fetched into `refs/tags`, so the
  Checking step names the copy `v3.0.52` instead of a bare sha.
- **CI runs once per wave** (hub HUB-065, `governance/WAVES.md`). `checks` runs on a pull request, on
  `workflow_dispatch`, and on a push to `main` only — no longer on every push to every branch. On
  `main` a `gate` job looks for a green pull-request run of `checks` on the same commit and skips the
  tests when there is one; the run still ends `success` for the release gate. Anything the gate cannot
  establish runs the tests.
- **Windows:** the update fix above was confirmed on the Windows VM; `install.ps1`'s one changed line
  (the post-install `autosound_ai.py doctor` hint) has not been run on a VM. The Windows CI job passed
  on the branch.

## [v3.0.52] — 2026-09-14 · the beta channel gets its own copy of the method

> **Upgrading — only `--channel beta` changes, and no candidate has been published yet.**
> `install.sh --channel beta` / `install.ps1 -Channel beta` no longer move the terminal's copy of the
> method: it stays on the newest release, and the candidate goes into a second checkout,
> `~/.claude/skills/.autosound-tuning-beta`, that no link points at. A stable install changes nothing.
> For a front-end: run that copy by path and declare it in `AUTOSOUND_SKILL_ROOT`
> (`references/tooling/installation.md`, "Two channels on one machine").

- **The method's beta channel is a second copy, not the one copy moved** (hub #145, TCC-011). The
  user's rule: a Claude session in the terminal runs a released tag; a session the app starts with its
  beta box ticked runs a candidate. Both read one checkout until now, so `--channel beta` put the
  candidate in front of the terminal too. The installers now keep `~/.claude/skills/.autosound-tuning-src`
  on `v3.*` whatever the channel, and on beta clone or move `~/.claude/skills/.autosound-tuning-beta` to
  the newest of `v3.*` and `beta-v3.*`; `--uninstall` removes both.
- **`deployment.py` checks a declared copy.** With `AUTOSOUND_SKILL_ROOT` set, the copy it runs from
  and the project's link must be the declared checkout (exit 3 otherwise, 4 when the declared one has
  no identity), and a personal copy at another commit is reported as another channel instead of
  refused. Unset, nothing changes — so a beta session that forgets the variable is refused at step 0.
- `scripts/installer-consistency.py` compares both checkout paths between the two installers, and
  `--print SKILL_SRC` / `--print SKILL_BETA_SRC` give them to a consumer.
- **Windows:** `install.ps1`'s half has not been run on a VM yet.

## [v3.0.51] — 2026-09-13 · installer-consistency.py runs on Windows

> **Upgrading — nothing changes for a tune or an install.** For TCC this is the tag to re-pin to: its
> Windows run of `scripts/installer-consistency.py` was red on `v3.0.50` and is green from here (hub #144).

- **`scripts/installer-consistency.py` runs on Windows** (hub TCC-010, #144). v3.0.50's beta-channel order check ran `bash`, which on Windows is often WSL's launcher (`System32\bash.exe`): with no Linux installed it answered with a UTF-16 notice, the check reported six false divergences on GitHub's `windows-latest`, and TCC's Windows CI went red. On Windows the checker now takes Git for Windows' own bash, found beside `git`, never the launcher, and says so in one line when there is none; the function and its input go in on stdin as bytes. A `windows-latest` job in `checks` runs the triplet check, so Windows is checked before a tag.

## [v3.0.50] — 2026-09-13 · a beta channel in the installers, and group delay through the direct sound

> **Upgrading — nothing to do for a tune in progress: no number `rew_tool` already computed changes.** What a
> caller may notice across `v3.0.49 → v3.0.50`:
> 1. **The measurement report prints a second group-delay block.** `rew_tool.py` now pulls the impulse
>    before the group delay and prints "Group Delay — крізь FDW-6" beside REW's, whose header now names
>    its window. A reader parsing that text sees one more block and a renamed header.
> 2. **`install.ps1` under the one-liner no longer ends the PowerShell session when it stops early.** The
>    code is left in `$global:AutosoundInstallExit` and `$LASTEXITCODE`; run as a file it still exits with it.
> 3. **`scripts/installer-consistency.py --print` has two more names**, `SKILL_BETA_GLOB` and
>    `TCC_BETA_GLOB`; the six before them are unchanged.
> 4. **`install.cmd` fetches `install.ps1` from this tag.** It pointed at `v3.0.46` since HUB-030, so a
>    Windows user without a local `install.ps1` got the installer of three releases ago; the triplet check
>    now holds that tag to the release like the README lines.
> 5. **`scripts/commit-lang.py` ships here** — the commit-msg hook TCC can install from its submodule (hub #133).

- **Beta channel in the installers** (hub HUB-060): `install.sh --channel beta`, `install.ps1 -Channel beta` and `install.cmd -Channel beta` take the line's releases and its release candidates `beta-v3.*-rc*`, for the method and for the app, newest by hub RELEASE-CHANNEL.md §11.2 — a release above its own candidates, candidates of one version by number. Stable stays the default and unchanged; `--skill-ref` / `--tcc-ref` still win. Run on a Windows (ARM) VM: the method and the app both resolve on the beta channel, and an unknown channel is refused with exit code 2.
- **The Windows installer no longer closes the user's PowerShell window when it stops early** — `-Help`, an unknown `-Channel`, a finished `-Uninstall`, "no" at the prompt, git still missing. Under the README one-liner (`irm … | iex`) there is no file, and `exit` ended the user's own session over the line that said why; the script now returns there and leaves the code in `$global:AutosoundInstallExit`. Run as a file (`install.cmd`, `-File`) it still exits with the code, and `install.cmd`'s download path reads the variable. `scripts/installer-consistency.py` fails on a bare `exit` in `install.ps1`. Run on the same VM: `irm … | iex` answered "no", `-Help` and `-Channel bogus` leave the window open (exit code 2 in `$LASTEXITCODE`), and a `-File` run still exits with 2.
- **`scripts/tag-check.sh --candidate vX.Y.Z`** checks a release candidate: its entry may still be `## [Unreleased]` and its manifest the previous version — the release commit does that bookkeeping (hub #141) — and the hub preflight names the next `rcN`. The release mode is unchanged. CI is asked once, by the hub preflight's `ci-green`: `tag-check.sh`'s own CI step is gone (hub HUB-059).
- **`scripts/commit-lang.py`**: a commit-msg hook that refuses a message with Cyrillic in it (`--install-hook`); `CLAUDE.md` carries the English-on-GitHub rule.
- **Resonalyze ports re-pinned on `23f2e70`** after reading the #187 diffs (hub PAS-007): header lines in `dsp_math.py` and `resonalyze_vc.py`, no behaviour change.
- **Group delay through the direct sound** (hub RES-010): `windows.windowed_group_delay` reads τ = Re[T·conj(H)]/|H|² through the same gate or FDW as `windowed_spectrum` (the definition is Resonalyze #186, pinned in its header); `rew_tool.py`'s measurement report prints an FDW-6 block beside REW's group delay, each named by its window; `analysis-playbook.md` reads junction timing through it. Across seven hand-held positions on `_49`, τ through FDW-8 scattered ×0.16–0.29 of REW's own group delay; validated 800–5400 Hz. No new measurement: it reads the impulse already captured.
- README: the 3rd place at the German EMMA Final 2026, in all four languages.

## [v3.0.49] — 2026-09-11 · the reviewer is one channel with three tasks, and it names no model

> **Upgrading — the reviewer channel changes under a caller; nothing in `rew_tool` does, so a tune
> in progress is unaffected.** Four things to know before you move across `v3.0.48 → v3.0.49`:
> 1. **Name the reviewer's model — there is no default any more.** `GEMINI_CRITIC_MODEL` (or
>    `AUTOSOUND_CRITIC_MODEL`, any vendor). Unset, `gemini_critic.sh` prints `agy models` and exits
>    **3**; `autosound_ai.py` does the same with the CLI's list when there is no key (it used to
>    take the list's first id). A setup that relied on the built-in `gemini-3.1-pro-high` stops
>    until one line is added to `~/.config/autosound/critic-env`.
> 2. **The advisor's model variables are no longer read** — `GEMINI_ADVISOR_MODEL`,
>    `AUTOSOUND_ADVISOR_MODEL`, `CLAUDE_ADVISOR_MODEL`, `CODEX_ADVISOR_MODEL`. The Advisor uses the
>    one model; a value left in one of them is named on stderr, not obeyed.
> 3. **A fallback model runs only if you name one** (`GEMINI_FALLBACK_MODEL`). A dry quota is now
>    reported instead of silently switching to a weaker model.
> 4. **An unknown model name is a stop, not a reply.** agy's `invalid model selection` used to be
>    printed as if it were the review; the wrapper now lists what agy can run and exits **3**.
>
> For a front-end (TCC): `critic` and `advisor` keep working as they are called today, and the
> answer's marker names the task — `— [critic|advisor|ask: <model>]`. The second model slot can go;
> `ask` needs no project files, so it answers in a folder the intake has not reached.

- **One channel, one model, three tasks (the Arbiter's ruling; skill#27, hub SKL-032).** The Critic
  and the Advisor were two wrappers with two prompts and two model slots, and the split cost a day:
  TCC reported "the critic does not answer", every fix went into the critic, and the one failing
  was the advisor — `autosound_ai.py` looked for the advisor's own model variable, and TCC sets
  only the critic's. Now the tasks differ in the question and its wording and nothing else:
  **critic** checks a proposal, **advisor** searches for a solution to an open question (both
  tuning, regulated — the tuning contract and the project context are required, the reviewer's
  memory file is read whenever it exists), and **ask** is new: any plain question — a translation,
  a letter's wording — under the interaction contract alone. `*_advisor.sh` is the critic's wrapper
  with the advisor task; `ask` is `AUTOSOUND_REVIEW_TASK=ask` or `autosound_ai.py ask`.
- **An interaction contract, separate from tuning — `assets/interaction-contract.md`.** Roles,
  tasks, how to answer, language, and what a text meant to leave the project must not carry. It
  says nothing about tuning (the selftest holds it to that) and goes into every call; the tuning
  rules moved to their own layer (`scripts/reviewer-tuning.txt`) that only the tuning tasks get.
  Four copies of two prompts became one set of files that the bash wrappers and `autosound_ai.py`
  both read.
- **No model name in the code (skill#27).** `--doctor` went red on a healthy channel: its smoke ran
  the built-in fallback `gemini-3.5-flash-medium`, a generation agy no longer offers, and never the
  model the Arbiter had chosen. The literals are gone — a table of model names is a promise to keep
  updating it (the same reason `autosound_ai.py` dropped its own) — and the user's rule of
  2026-09-08 decides instead: not named → the list, and stop. `--doctor` smokes the named model and
  tells "the CLI answered" from "the name is unknown", with the list beside it.
- **The feedback gate has a second channel — `post_feedback(..., channel="tcc")`.** A finding about
  the TCC window had nowhere to go: the gate knew one repo, the model refused (correctly) and the
  report landed on the skill. The caller names a channel from a closed set; the repo behind it is
  hardcoded, the returned URL is verified against that repo, and a repo written out by hand is
  refused before anything runs. The default is unchanged.
- **Found on the way:** outside a project folder the bash wrappers printed two `No such file` lines
  while writing the audit trail (only reachable now that `ask` runs there) — silenced.

## [v3.0.48] — 2026-09-11 · the index no longer hides an Upgrading note written as a sentence

> **Upgrading:** documentation and one checker; nothing in `rew_tool` changed, so a tune in
> progress and every caller are unaffected by THIS patch. **But if you took `v3.0.47` and read the
> index to learn what breaks, it told you nothing does — and something does.** The `v3.0.47` row
> said `—` where it should have said `yes`: its Upgrading note is real, and one item in it breaks a
> caller (`Process.skip_step(step_id)` with no reason now raises `ProcessError`; the CLI takes
> `skip <id> <reason ...> [--superseded-by ID]`). Re-read that note before moving a front-end
> across `v3.0.46 → v3.0.47`. This patch carries the corrected row — the number is spent on
> purpose, by this file's own rule: a warning the index hid is a reason to cut `x.y.z+1`, never to
> move a tag.

- **The Upgrading flag read one punctuation mark — `scripts/changelog-index.py` (autosound-hub
  `TCC-009`).** The index generator tested for the literal string `**Upgrading:**`; the `v3.0.47`
  note opens `> **Upgrading — four things change under a consumer…**`, so the flag came out false,
  and `--check` stayed green, because it compares the table with what the same code regenerates —
  both halves wrong in the same way. Reported by TCC while bumping its pin: they read the index to
  learn what breaks, and the index said nothing. The flag now reads the note's OPENING LINE —
  colon or dash after the word, quoted with `> ` or not (the file already carried all three forms:
  fifteen quoted with a colon, two unquoted, one with a dash) — and a mention in the middle of a
  sentence is prose, not a note. A line that opens with `**Upgrading` in any OTHER form is refused
  by name, in the writer and in `--check` alike, because a silent `—` reads exactly like "nothing
  to know". The selftest covers the three forms, the mention, the refusal (nothing written) and its
  repair; the live proof on a copy of this file — `> **Upgrading** four things…` →
  `CHANGELOG.md:133: v3.0.47 opens an Upgrading note in a form the index cannot read`, exit 1 on
  both paths, exit 0 on the untouched copy.
