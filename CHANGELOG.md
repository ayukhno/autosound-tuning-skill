...
> 5. **Above 1 kHz a mid↔tweeter delay is confirmed on a measurement of the pair.** Where the sum and the arrival
>    reading disagree by a whole cycle, the desk no longer picks one: the joint comes back UNVERIFIED with both
>    candidates, and one sweep of the two drivers together settles it. Below 1 kHz nothing changed.
...
- **`predict --align` does not overrule a tune above 1 kHz** (research RES-013, hub #168): a whole-cycle
  ...
  right side, where the old code would have proposed an answer 4–5 cycles away. Below 1 kHz unchanged.
