...
### Added
- **Deviation analysis in the curve visualizer:** a new "Analyze" panel picks a Baseline (reference) and a Compare curve — same math for Measured-vs-Target or two ALL runs before/after a change — and computes an autonomous, no-AI band-by-band deviation report: broadband tilt (100 Hz–8 kHz fit), coverage %, Broad-tonal/Resonance-control grades, and per-band PEAK/DIP/NULL features (prominence-based detection, width-classified — narrow dip = likely acoustic null, don't chase; narrow peak = PEQ-fixable resonance; broad = tonal, gentle wide EQ is fair). Reuses the existing 10-band `FREQ_BANDS`/`BAND_TREND` grid and computes "instruments affected" live from `BAND_INSTRUMENTS`. Smoothing: None/1-12/1-6 (default)/1-3 octave or a psychoacoustic variable-octave mode (narrow at bass, ~1/3 oct above 1 kHz). Level is aligned by default (auto 300 Hz–3 kHz anchor + manual fine-tune nudge, since gain is always re-optimized separately) so the report reads shape/EQ, not gain. A delta-bar strip renders under the main chart on a fixed ±10 dB scale, and the two curves under analysis are drawn with the same smoothing/level-shift as the report so the chart and the report always agree. No L/R triangle — without time-alignment there's nothing actionable to do with an L/R difference, so it isn't analyzed. Two-step flow (pick curves → press Analyze); changing a setting afterward requires pressing Analyze again to refresh.
