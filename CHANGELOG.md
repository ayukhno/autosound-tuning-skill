...
- **Between tags, work lands under `## [Unreleased]`, and a change the user must act on says so under
  ...
  stable channel never sees it. The release tag lands on the newest candidate's commit, so the last
  candidate already carries the manifest bump and `## [vX.Y.Z]`.
...
- **`scripts/tag-check.sh --candidate vX.Y.Z`** checks a release candidate: its entry may still be `## [Unreleased]`, the manifest must already say X.Y.Z, and the hub preflight names the next `rcN`. The release mode is unchanged.
