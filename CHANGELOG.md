...
- **`scripts/tag-check.sh --candidate vX.Y.Z`** checks a release candidate: its entry may still be `## [Unreleased]` and its manifest the previous version — the release commit does that bookkeeping (hub #141) — and the hub preflight names the next `rcN`. The release mode is unchanged.
