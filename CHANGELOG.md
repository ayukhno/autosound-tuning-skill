...
- **Between tags, work lands under `## [Unreleased]`, and a change the user must act on says so under
  ...
  candidate for a patch — whose entry has a Breaking section, because that is not a patch. **A minor
  or major is tried before it is released:** `scripts/tag-check.sh --candidate vX.Y.Z`, then the tag
  `beta-vX.Y.Z-rcN`, which only `install.sh --channel beta` / `install.ps1 -Channel beta` take; the
  stable channel never sees it. The release tag lands on the newest candidate's commit plus only the
  release's bookkeeping — the rename to `## [vX.Y.Z]`, the manifest and the install pins (§11.3).
