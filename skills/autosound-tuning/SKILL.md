...
* **Keep the state on disk as you go (simple, not a bureaucracy).** When a change is agreed, bank it with `apply.propose` — it writes the versioned snapshot AND emits the clean settings sheet the Arbiter enters; after the Arbiter applies it, `attest` (🟡→🟢). Useful, not policing: A/B, revert, resume after `/clear`, one honest audit trail. ⚠️ This substrate is the **Arbiter's pull-tool**, not a Generator self-discipline ritual — the Arbiter enters **only tool-emitted settings-sheets** (each cites a `v_NNN` snapshot, a 5-second disk check) and any "saved/done" claim costs a path. Why pull beats self-discipline, the operating modes, and the spot-check habit → [`process-control.md`](file:///skills/autosound-tuning/references/core/process-control.md).
* **Skill seems to be MISSING a tool, or acting AGAINST its own docs → verify first, don't spiral:** The skill is installed via a **symlink**, and a plain `find` does **not** traverse it on macOS → a false "this file/tool doesn't exist". **Before concluding anything is missing or broken, re-check with `find -L` / `ls` / the CANONICAL repo path** (`…/dev/Claude/autosound-tuning-skill/skills/autosound-tuning`), never a symlink-blind `find`; when the code contradicts the docs, read the actual source. A symlink-blind read can silently hide *arbitrary* skill content, so treat your whole view as **partial** and re-load from the canonical path before trusting conclusions reached earlier in the session. If it is a **real** discrepancy: **ASK the Arbiter, don't decide unilaterally** — *"here's what's off; I can fix/work around it locally, or we wait for the author — which do you want?"* → **"fix it"** = apply the fix **and** log one line in `rew_analitic/skill-inbox.md`; **"wait"** = file a GitHub issue via `gates/side_effect.py` `post_feedback` (`field-feedback` template; repo hard-coded, returned-URL verified, FAIL LOUD) and **pause that thread**. Never invent a silent workaround. Skip the ask only at the extremes (trivial + certain → silent inbox note; hard blocker with no workaround → file + tell the user to wait). Full incident + why the damage is systemic → [`installation.md`](file:///skills/autosound-tuning/references/tooling/installation.md#troubleshooting).
