...
* **REW's view is the Arbiter's.** Never change a measurement's smoothing, or any display setting, to read it — ask the reader for the smoothing it needs (`rew_api.get_fr(mid, smoothing="None")`); a session stopped between "off" and "back on" leaves the view changed (hub TCC-015).
