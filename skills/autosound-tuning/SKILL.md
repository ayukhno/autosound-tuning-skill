...
* **Don't rebuild tools you already have:** EQ transfer + analysis already ship in `rew_tool/` (`atf_eq.py` Helix export · `rew_api` `set_filters`/`set_equaliser` · `analysis.py` `compute_deviation`/PEQ). Check `rew_tool/` **and the project** for an existing script before writing a new one — a real failure mode was re-creating helper scripts each session after forgetting them.
