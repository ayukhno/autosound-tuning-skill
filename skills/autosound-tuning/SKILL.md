...
* **Skill seems to be MISSING a tool, or acting AGAINST its own docs → verify first, don't spiral (Lesson 2026-07-08):** The skill is installed via a **symlink** (`~/.claude/skills/autosound-tuning` → the repo), and a plain `find` does **not** traverse a symlink on macOS → a false "this file/tool doesn't exist" (real incident: `rew_tool/target_bands.py` declared a "gap" while it existed and was documented as the solution; the whole session then went the wrong way on that false premise). **Before concluding anything is missing or broken, re-check with `find -L` / `ls` / the CANONICAL repo path** (`…/dev/Claude/autosound-tuning-skill/skills/autosound-tuning`), never a symlink-blind `find`; when the code contradicts the docs, read the actual source. ⚠️ **The damage is SYSTEMIC, not one file:** a symlink-blind search silently hides *arbitrary* skill content across BOTH tiers — the always-loaded **sample-rate** guardrail here (DSP-agnostic: samples at the wrong native rate ruin the alignment) *and* the equipment-specific value it points to (`knowledge/dsp/helix-dsp-ultra-s.md`: Helix native rate = 96 kHz) — none of which the user ever saw flagged. So the moment you notice a symlink-blind read, treat your whole view of the skill as **partial**: re-resolve the install and re-load from the canonical path, and treat conclusions reached earlier this session as suspect until re-checked — don't just patch the one visible symptom. Then split by outcome:
  * **Default when it's a real skill discrepancy — ASK the Arbiter, don't decide unilaterally:** surface it plainly — *"here's what's off; I **can** fix/work around it locally, or we wait for the author's advice — which do you want?"* Then honor the answer:
    * **"fix it" →** apply the local fix **and** log a one-line note in `rew_analitic/skill-inbox.md` (for info / later folding into the skill).
    * **"wait" →** file a GitHub issue (autofilled: symptom · what you verified · expectation vs fact · skill version/commit · OS) via the `field-feedback` template, routed through `gates/side_effect.py` `post_feedback` (repo hard-coded, returned-URL verified, FAIL LOUD; triage side = `scripts/issue_triage.py`), and **pause that thread until it's resolved** — never invent a silent workaround or proceed as if it's fine.
  * **Skip the ask only at the two extremes:** trivial + you're certain (an obvious self-resolved lookup) → just the silent one-line inbox note, don't derail the user; **or** a hard blocker with **no** viable local workaround → file the issue + tell the user plainly to wait (there's no real choice to offer). *(The issue-triage/auto-answer side is intentionally still WIP — asking + filing + honest "wait" is the part that must always happen now.)*
