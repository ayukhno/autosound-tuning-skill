...
- □ **A broad LEFT-side excess at 250–430 (near-side gain, LHD) carried by BOTH branches** — measured w +4.7 / m +5.2 dB L−R (2026-07-15), same cabin physics seen at ~247 Hz in an independent preset on this car. Boominess + lower-mid left pull; treated with per-branch OUTPUT cuts (not a virtual-layer cut — the critic's direct/reflected argument), plus narrow modal ridges (L 188, R 134) beside the door null. ⚠️ Cutting this zone drops the BASS IMAGE's height — see `staging-depth.md §8` for the symmetric mid-pair compensator. → your car: check both branches before assigning the excess to one.
...
- □ **Rear-fill worked config (validated first pass, 2026-07-15):** differential ±50% on the rear virtuals · HPF 315 LR24 / LPF 4000 LR24 · PK 850 −4.5/−5.5 Q1.5 (the rears' own hump) · delays 12.0/9.5 ms (= measured front→rear arrival diff + ~9 ms Haas; the sides differ because the rears sit asymmetrically to the driver) · outputs +3, level by remote-knob mute-contrast ladder. ⚠️ Delays/EQ are THIS install's arrivals/humps — derive yours from YOUR raw rear sweeps (`phase_5 §6`).

- □ **A presence VALLEY 2.5–6.3k (floor ~−6 rel 200–1k, ≈−2.7 vs target), symmetric, stable across sessions?** In the author's build this looked like crossover underlap (m LPF 2000 LR12 → tw HPF 3625 BW24) but measured COHERENT (seam coherence −1.5..−2.8 median, both skirts within −8 dB of their cores) → a virtual-layer PK lift verified nearly loss-free (+2.32 measured of +2.2 entered; agreed escalation criterion: measured < +1.4 of +2.2 would have meant incoherent → corner surgery). Lifting it raised the stage off the dash. → measure YOUR seam coherence before choosing EQ-lift vs corner-move.
- □ **LEFT-side cold interference lobes ~700–880 and ~1.4–1.8k, POSITION-SENSITIVE (2–5 dB between mic setups), plus a hot L lob 990–1250?** In the author's build the seesaw (±3.5 dB per 1.5 oct) made left-of-center piano wander with pitch and the bass line drift left when the center was off; trimming the hot lob (an m_L PK cut that also UNWOUND net pair phase, `review-loop.md` triage) fixed both; the cold lobes are interference (boost forbidden), audible in a solo mute-test but masked by the phantom in stereo. → map your own L−R at 1/6-oct across 0.7–1.8k before blaming drivers.
- □ **Bass pan collapse on a pan test (bass "from both sides"), mids panning clean?** The author's mechanism: the door-null seesaw (w solo L−R +1.9 / −9.7 / −0.3 across 90–250) + the mono sub below 88 — `diagnostic §31`. Trimming the far side's 160–215 ridge helped the near-pan side and improved the w↔m joint; the rest is modal ceiling. → run the per-band solo L−R table on YOUR car first.
