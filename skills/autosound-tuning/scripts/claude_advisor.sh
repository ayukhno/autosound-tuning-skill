#!/usr/bin/env bash
# claude_advisor.sh — Claude-based "Advisor-Expert" channel (depth/staging work).
#
# Variant of claude_critic.sh where Claude plays ADVISOR-EXPERT,
...
#
# Adds SESSION MEMORY: a persistent file is injected each call so Claude accumulates
# knowledge/decisions across rounds and sessions. Append to it after each round.
#
# Usage:
#   claude_advisor.sh <package.md> [trace.csv]
#
set -euo pipefail
SCRIPT_NAME="claude_advisor"
. "$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/_claude_common.sh"

# Preflight: `claude_advisor.sh --doctor` diagnoses the channel + runs a live smoke.
if [[ "${1:-}" == "--doctor" ]]; then claude_doctor && exit 0 || exit 1; fi

ADVISOR_MEMORY="${ADVISOR_MEMORY:-$PROJECT_MIRROR/depth-advisor-memory.md}"

PKG="${1:-}"; TRACE="${2:-}"
[[ -n "$PKG" ]] || die "usage: claude_advisor.sh <package.md> [trace.csv]"
[[ -f "$PKG" ]] || die "package not found: $PKG"
claude_preflight

PRIMARY_MODEL="${CLAUDE_ADVISOR_MODEL:-}"
...
claude_run "$PRIMARY_MODEL" "$PROMPT_FILE" "advisor"
