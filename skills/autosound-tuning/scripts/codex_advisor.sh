#!/usr/bin/env bash
# codex_advisor.sh — the ADVISOR task on the one reviewer channel (codex_critic.sh).
#
...
AUTOSOUND_REVIEW_TASK=advisor exec "$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/codex_critic.sh" "$@"
