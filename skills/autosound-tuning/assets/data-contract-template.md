...
- **The critic channel:** Claude Code (the orchestrator) + `scripts/autosound_ai.py` (the reviewer's key or CLI, e.g. `agy`) — on the same host, no VM. Setup → `references/tooling/setup-critic-channel.md`.
