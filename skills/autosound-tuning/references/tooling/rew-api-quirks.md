...
- **The API is measurement-scoped: NOTHING reports which `.mdat` is open.** A measurement record carries `title`, `uuid`, `notes` and `date`; no endpoint **these tools speak** (`rew_tool/rew_api.py`, and the routes listed in this file) reports a session, project or file, so a pull cannot discover its own source. The session name lives in REW's title bar and in the operator's head. Stated the same way, from live use, in `research`'s `experiments/code/resonalyze-cross-check/pull_irs.py`. Scope of the claim, said plainly: this is the endpoint set we drive — it is not a sweep of REW's full API surface, so treat it as "we have no way to ask", not as "REW provably cannot tell you". Consequences, both deliberate:
  - `rew_tool/resonalyze_ir.py` **requires** one of `--session PATH` / `--session-name NAME` / `--session-unknown`, and refuses to run without one. There is no blank state on purpose: an empty session field reads as recorded while saying nothing, which is worse than "unknown".
  - `--session PATH` is **checked, not just recorded**. An `.mdat` is `Java serialization data, version 5`, so a measurement's `uuid` sits in it as the same sixteen raw bytes `UUID(...).bytes` gives, and a plain byte search finds it **without parsing the format**. Every measurement's uuid must be in the named file or the run refuses before writing anything (115/115 over a 373 MB session in ~9 s, 2026-08-31). What that proves is "this measurement **is in** this file", not "was taken from it" — a later save of the same session contains the earlier one. That weaker claim is the one worth having: it answers *which file must be kept*, which is when the bill actually arrives (hub `CAR-001`).
  - ⚠️ Reading the **data** out of an `.mdat` is a different and unpriced problem: the identity half is a byte search, the measurement half would mean re-deriving REW's timing semantics from a Java blob coupled to a beta. Don't confuse the two — only the first is done here.
