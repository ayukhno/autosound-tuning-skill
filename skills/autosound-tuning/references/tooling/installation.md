...
- **Reviewer CLI inside an agent session.** A reviewer CLI (agy/claude) started from *inside* another agent session used to hang (agent-inside-agent), because it inherited the session's markers. `autosound_ai.py` now starts it with those markers removed, so a TCC-launched session reaches the reviewer too. A CLI that does not answer is stopped after a wait that grows with the job (600 s at least; `AUTOSOUND_CLI_TIMEOUT` overrides it), and the wait is printed before it starts. See [setup-critic-channel.md](references/tooling/setup-critic-channel.md) §3.
