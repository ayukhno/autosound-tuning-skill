...
* **`rew_tool/rew_tool.py`**
  * Main CLI entry point. Two modes:
    ...
    * **`time-offset [seconds]`** — get/set the REW measurement **Time Offset** via API (`GET`/`POST /measure/timing/offset`) — the Phase-1 "set a shared Time Offset before reading phase" step, no more GUI trip. Safe (no capture).
    * **`capture-sweeps`** — **safety-gated** mass loopback-sweep capture (verified feasible against REW 0.9.5: `POST /measure/command {"command":"SPL"}` + `POST /measure/naming`). **DRY-RUN by default** — it names each `<ch>_<ver> (sw)`, pulls each channel's HPF from the active slot's crossovers (`state/`), runs `presweep_safety.require_safe` for **every** channel (`--fs "tw-L=1400,…"` supplies driver Fs, `--level`/`--headroom` the sweep level/clip check), and prints the plan; **one unsafe channel aborts the whole batch, no waiver**. Only stationary loopback sweeps are automatable — **MMM/RTA stays manual** (moving mic). `--fire` is present but **hard-disabled** (`_LIVE_FIRE_VALIDATED=False`) until a supervised single-channel live test validates the fire sequence on the rig — until then `--fire` falls back to DRY-RUN. `--only "tw-L,m-L"` limits channels.
  * **Capture API wrappers** (`rew_api.py`, verified live on REW 0.9.5): `get_measure_commands`, `get_sweep_config`, `set_measurement_naming`, `measure_command` (⚠️ `"SPL"` fires a sweep — gate it), `get_timing_offset`/`set_timing_offset`.
    * `python3 rew_tool.py selftest` = offline check of batch + joints + capture paths (no live REW).
