...
* **`rew_tool/dsp_math.py`** — REW-exact filter math shared by `xover_select` (BU/BE/L-R crossovers — scipy Bessel with `norm="mag"`; PK/LS_Q/HS_Q ≡ RBJ; `apf2_response` hardware-verified on the Helix Ultra S, `helix-phase-allpass.md`; **`apf1_response(freqs, f0)`** — the 1st-order all-pass, −90° at `f0`, 0 → −180° overall, a pure 1/(π·f0) delay far below `f0`; the two share one convention, `apf1² ≡ apf2(Q 0.5)`). `eq_complex` dispatches by band kind — `PK`, `LS`/`LSH`, `HS`/`HSH`, `APF1`, `APF2` — and **refuses an unknown kind** instead of drawing it as a shelf (SCR-050). Verify: `python3 dsp_math.py selftest`
