...
> 🧩 **PATTERN REPOSITORY — starting hypotheses, not rules.** Everything here is "in similar conditions this often worked": a **starting point**, validated by measurement/ear, never a mandate. (Physics stays firm; the prescriptions are hypotheses.) See [`knowledge-architecture.md`](references/core/knowledge-architecture.md).
