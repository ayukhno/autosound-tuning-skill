...
1. **Language** (`project-intake.md` §0) — ask EN/UK/DE/PL **unless the front-end already says which** (`get_tcc_state.language`); the dialogue AND every project file follow it. Either way the step is **closed with what was decided as its evidence** — an answered question left open is asked again next session.
