...
**✅ Quality gate → Phase 0:** language set; `autosound_context.md` (Engineering Profile) created — `preference-profile.md` may start as a stub, since taste is asked in Phase 0 after the baseline and applied in Phase 5; **the machine files exist and validate** — `project.json`, `dsp_profile.json`, the glossary, and a first ledger snapshot with every profile-declared tier populated (`python3 rew_tool/contract.py check <project> --gate` exits 0 — **`--gate`, not plain `check`**: plain `check` answers "is anything here wrong", which an EMPTY project satisfies, and this gate is asking whether everything it needs exists) — a prose-only intake is not a complete one, since a consumer front-end has nothing to render without them; install verified + protective HPFs set for fragile drivers. The target curve and the goal are **optional** here (the Arbiter, 2026-09-22): the intake form offers them (`goal.target_curve`, purpose, music, wishes), and what the person chose there stands. **Whatever is still empty is asked in Phase 0, after the baseline** (`phase_0_baseline.md` §2). Nothing in the baseline capture reads them, a curve is never defaulted, and `enter-phase 1` refuses without one.
...
1. **Language** (`project-intake.md` §0) — the session writes in the **interface** language, always (the Arbiter, 2026-09-22), and it is **read before it is asked**: the front-end's report wins (`get_tcc_state.language`; the intake form is started with it as `--lang` and writes it on Save without asking), else `project.json`'s `language.reply` (`python3 rew_tool/project.py <project> language`), else ask EN/UK/DE/PL. The dialogue AND every project file follow it. **Write the answer** — `intake.save(<project>, "project.language", "<code>")` — so a session after a `/clear` reads it instead of coming back in English (S-045); the step is then **closed with `project.json` as its evidence**, an answered question left open being a question asked again next session. The USER's own language (`language.user`, optional on the form) is recorded and switches nothing, and the language the person TYPES changes nothing either.
...
3. **Interview — only what the first measurement needs** (`python3 rew_tool/intake.py fields --now`): the car (four parts + LHD/RHD; a car the skill has seen can be picked and filled in, and an edited one is a new car), **the reference seat** (it decides where the mic stands), the DSP's vendor and model (the bundled profile answers its checklist on an exact match; a processor it does not describe is a **new processor**, and only then are its base questions asked, once), which of its tiers this car uses, the channel codes, and the timing reference (a physical loopback or an acoustic one) → write `autosound_context.md`. The goal, the curve, the genres and the test-track libraries are asked on the same page as an optional block. **Not asked at all**, per the Arbiter's review of 2026-09-22: whether each output can be soloed (always yes), the mic model and its calibration files, the signal chain (source, inputs, amps), and the input clip check; a tool reads them, or the session raises them at the step that needs them (`intake.PLACES`, the page's memo). A pre-selected default (LHD, a new tune) is confirmed, not asked; the seat has none, because it is written once.
...
TCC's: `python3 rew_tool/intake_form.py serve <project> --lang <interface language>` (TCC, or the
skill at start, passes the language it runs in; the page never asks it) puts on one local page only what starting
...
reference seat) is REQUIRED before the first measurement.** Purpose, music, the curve and the free
wishes (#1, #3, #6, #7) are on the intake form as optional: what the person answered there stands,
and Phase 0 asks after the baseline only what is still empty (the Arbiter, 2026-09-22: «хай там і
буде. але якщо не задано, то так запитати»). Stage priorities and test tracks come in Phase 4, taste
...
6. **Choosing the target curve — TOGETHER with the user, there is NO default.** Walk through the **curve→character** table (`voicing-by-ear.md`), narrow it by genres/taste to 2–3 candidates, finalize via the curve-audition method (same place) or take a candidate as the session's start and validate by ear in Phase 4/5. **A curve = a start and a shape, not a finish and not a level** (`naming-and-structure.md §6`). ⚠️ A curve picked on the form is the person's choice and stands; with none picked, it is chosen AFTER the raw baseline (Phase 0), with the measured reality in hand. Either way the person may change it once they have seen the baseline. In a report that is «криву обираємо попередньо, остаточно — після замірів Фази 0»: the same rule as the ledger snapshot above, a metaphor the reader does not share is not a word to hand him (`naming-and-structure.md` §1a).
