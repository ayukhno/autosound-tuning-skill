...
Before any measurements are taken, establish the channel abbreviations (`sw / w-L/R / m-L/R / tw-L/R / c / r`) and the title grammar — its one home is [naming-and-structure.md §3](references/core/naming-and-structure.md): `(sw)` sweep, `(rta)` MMM RTA, `_N` the number of the measurement series (starts at `_1`, NOT "baseline"; a DSP state can have several), a free-form clarification welcome after the method (another measurement, the same series). *Examples:* `m-L_1 (sw)`, `w-R_1 (rta)`.
...
⚠️ **After the pass, run the post-sweep quality gate** — `python3 rew_tool/state/process.py <project>/process capture-check --session` (`--session` adds the whole-session table and the ctl1 → ctl3 drift record, written onto the round). It asks two things of every capture in the round: is it there and readable at all, and is its pre-echo far worse than the CLEANEST capture of the same driver (`REMEASURE_MARGIN_DB`, 15 dB). The second is why the comparison is a driver against itself rather than against a number: on a real car sweep the pre-echo includes the loopback reference and earlier arrivals, so an absolute threshold condemns good sweeps. A flagged capture is still readable — re-taking it is the Arbiter's call — but the step cannot close until the round has been checked (SCR-040). The pre-sweep gate protects the hardware; this one protects the conclusions.
  ...
  **Then say what the CHECK said, and stop** (S-038): how many captures are usable, what is flagged, what could not be checked and why, and the drift record. The arrivals are in the round, and reading them is **Phase 1's** work — `predict --align` and `arrival_triangulate` do it through a window, with the trust gate, the alias rules and the ILL-POSED verdict said out loud. A prose reading here has none of those guards, which is why a session that tried it had to hedge its own number («~1.2 ms later, ~41 cm, plausible for a left-hand-drive seat… a candidate for cross-check in Phase 1») — and a hedged number in the dialogue is worse than no number: the Arbiter carries half a conclusion into the phase that would have produced a whole one. The same reading came back in a FRESH session after the chat was cleared, so it is the text's habit, not one session's.
...
* **A centre, if the car has one: capture it like any other channel** (`c_1 (sw)` from the tripod, `c_1 (rta)`). ⛔ **The with/without-centre SUM is NOT a Phase-0 capture** (the Arbiter, 2026-09-19: measuring any sum here is absurd). Nothing is aligned yet — no delays, no tuned crossovers — so `ALL_1` would measure the unaligned system and answer a question nobody asked; and with protective filters below 1.1 × Fs the pre-sweep gate refuses the sweep anyway. It also contradicted this method's own capture plan, which lists Phase 0 as solos and the groups at Phase 2 (`naming-and-structure.md` §3). Take the pair where the whole system is already captured — the verify pass, `ALL_final` / `ALL+C_final` (`naming-and-structure.md` §3's capture plan) — or with Phase 2's group pass if the desk needs it earlier; there `predict`'s `all_plus_c` and `verify_prediction --all-plus-c` finally have an aligned state to check against. The centre has no side of its own, so nothing about its code places it in a sum; what makes it summable is the MEASUREMENT that exists (`predict` gives `ALL+C` and the `c↔FRONT` pair, and `verify_prediction --all-plus-c` checks what it adds). Take that pair, when its phase comes, under one condition — one signal on both inputs — and say so, because a coherent sum is an UPPER BOUND for material whose correlation is unknown: for music the centre carries only the correlated part. *(Its own voicing is still Phase 5; this is only so the desk can predict what the mic hears.)*
...
⚠️ **ASK FIRST: are there position measurements?** One sentence — «чи є у вас заміри позицій (еліпсоїд навколо голови)?» — before the map is built. On a live project the nine-position set was on the tuner's disk the whole time (`9points-mes.mdat`, 63 sweeps), the session substituted a moving-mic RTA because that was what it found, and wrote 43 rows marked «stays put — assumed». His question afterwards was «чому скіл сам не запитав про такий замір?» (S-047).

⚠️ **A run that produced ASSUMED rows ends with the REQUEST, not with the rows.** `flaw_map` already knows the question was not asked — it stamps every peak it could not check `no positions measured -- staying is ASSUMED, not shown` — and now prints what would settle them: the channels, `<ch> p1…p9_<N> (sw)`, and the `capture-start` line to open the round. **The phase does not close while such rows exist and no request is on record** (`enter-phase 1` refuses): either the round was opened, or the Arbiter's answer is recorded (`decision "the ellipsoid for <channels>" "<his words>"`) — a decision is an answer, an unasked question is not. Only a PEAK's verdict turns on the microphone moving; a `cabin_null` or a `non_min_phase` row asks for nothing here.
