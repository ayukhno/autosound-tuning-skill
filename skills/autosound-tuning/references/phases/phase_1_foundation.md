...
* **De-embed before reading phase (doctrine 2026-08-24).** The `_1` solos were taken behind protective filters, and those filters are in the recording. `python3 rew_tool/rew_tool.py analyze-joints --process <project>/process --ver 1 …` takes them back out from the capture round's record before any delay / polarity / APF is computed, and answers **`check`** for a channel nobody recorded — that is the Arbiter's question, not a number to enter. Since a front-end writes `OFF` or a filter for every channel it captures (ruling 2026-09-06), `check` means the record came from somewhere else, or did not get written; `OFF` itself is the default answer and asks for nothing. One home for the rule: [`project-intake.md §3`](references/core/project-intake.md).
