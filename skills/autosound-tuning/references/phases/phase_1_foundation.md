...
>
> ⛔ **And the DSP's own numbers must be on record:** `enter-phase 1` refuses while the profile has no `sample_rate_hz`, no `delay` step, or no `crossover_filters` for a tier whose `fields` declare `hp`/`lp`. Every delay in samples is computed from that rate — a rate nobody wrote down is a rate the next session assumes. Record with `dsp_profile.py set-field`, then `finalize`; `dsp_profile.py open-questions <project>/dsp_profile.json` lists everything still open.
