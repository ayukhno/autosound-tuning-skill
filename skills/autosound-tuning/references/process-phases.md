...
На віхах (фінал сесії/проєкту, після слухової роботи) Claude **проактивно** проводить закриваючий ритуал (повна логіка + правила обережності → `references/feedback-loop.md`):
1. **Коротке опитування результату** (3-4 питання: чи досягли цілей · що звучить найкраще · що ще муляє · загальне враження) → у session log + матеріал для фідбеку.
2. **Session-log дисципліна**: changelog / dsp-state / ▶️ ПРОДОВЖИТИ / skill-inbox (SKILL.md §Session log).
3. **Feedback-пакет автору скіла**: скіл пропонує і збирає (inbox + changelog + профіль + опитування) анонімний markdown-пакет «що спрацювало / що ні / нові прийоми / досвід по авто-DSP-залізу»; **користувач читає, редагує і відправляє сам** — нічого автоматично.
4. **Ненав'язлива подяка-донат** — ОБЕРЕЖНО: лише при позитивному результаті, один раз, без тиску, посилання на Sponsor-кнопку (пропустити, якщо профіль ще не активний).
