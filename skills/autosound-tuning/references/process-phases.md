...
- **Delays — TWO layers.** (1) **Gross / arrival TA** — equalise each driver's direct-sound ARRIVAL time (tape measure to the dust cap + the **IR FIRST FRONT** of the solo channel). This is **crossover-INDEPENDENT → set it EARLY, before the crossovers are final** (it IS the EMMA "TA by the peak starts" recipe); the single `_1` sweeps do double duty — raw roll-off for the crossover sanity-check AND the IR first front for gross TA. Reference = the **measured latest arriver** (don't assume it's the midbass; absolute IR time needs loopback). (2) **Fine / joint TA** — phase in the overlap for a flat SUM → **after** the crossovers, by summation / all-pass (`helix-phase-allpass`). So "crossovers first" gates only the JOINT layer; the gross-arrival layer goes early. ⚠️ API IR timing is **usable with the right method** (leading-edge first-arrival + a shared loopback reference; the SUB/LF is the weak spot → read it from summation) — not a blanket "don't trust it" (`rew-api-quirks` Timing, `diagnostic §10`). By-ear "verticals→center" on mono, the sub by a double bass. **A stereo pair's delays are set ONCE for centering; do NOT detune one side for an FR dip** (ITD 200 Hz–1.5 kHz → the image drifts; cabin dips are often SBIR/non-min-phase — can't be closed by time/EQ). Align joints with an **all-pass** (`helix-phase-allpass`), not by shifting the channel's delay. Image-center = LEVEL not time (Wehmeyer §16); a broadband shift = time (§15).
...
- **SWEEP (loopback)** — for: the impulse (preliminary delay alignment), distortion (choosing safe band ranges), + other graphs per `analysis-playbook.md`.
  ...
  > ⚠️ **Arrival-TA reference = the MEASURED latest-arriving driver — NOT a fixed convention.** A DSP can only ADD delay, so the **latest** arriver gets 0 added delay and every **earlier** driver is delayed to meet it. Find "who's latest" FROM THE MEASUREMENT — **don't assume the midbass is the reference/latest**: it often arrives EARLY (real bug: midbass under-delayed → arrives before the others → the fix is to delay the MIDBASS, not "everyone else"). ⚠️ Don't conflate this with the "midbass = reference" rule in `helix-phase-allpass.md` — that's the PHASE-tuning anchor (which channel you don't all-pass), a DIFFERENT question from who's physically latest in arrival.
