...
На віхах (фінал сесії/проєкту, або «сформуй фідбек»):
1. **Session-log дисципліна** як завжди: changelog / dsp-state / ▶️ ПРОДОВЖИТИ / skill-inbox (SKILL.md §Session log).
2. **Feedback-пакет автору скіла** → `references/feedback-loop.md`: скіл сам збирає (з inbox + changelog + профілю) markdown-пакет «що спрацювало / що ні / нові прийоми / досвід по авто-DSP-залізу»; **користувач читає, редагує і відправляє сам** — нічого не йде автоматично.
