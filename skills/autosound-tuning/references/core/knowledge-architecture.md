...
| 5 | **Project State** | **machine files first** — the ledger `state/versions/v_NNN.json` (`state/<preset>/…` on a project still numbered per preset) · `process/process-state.json` + `journal.jsonl` · `project.json`; `tuning-changelog` · `audit-trail` · `dsp-state-current` are the human-readable views beside them | dynamic, per-session | measurements, decisions, history, current DSP state |
