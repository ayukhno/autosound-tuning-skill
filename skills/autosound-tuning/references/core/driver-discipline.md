...
4. **The reviewer is a stateless call, never the session judging itself.** Under TCC the session calls TCC's `call_critic` tool, which runs `autosound_ai.py` outside the session with the Arbiter's picked model and binary. Elsewhere `autosound_ai.py` runs the reviewer CLI from inside the session with the session's markers stripped and a bounded wait announced before it starts (skill #54); `--via api|cli|clipboard` picks the route for one run. The bound is the lesson of the old rule: a reviewer CLI spawned inside a session used to hang (~15 of 20 field sessions), and every hang became a "did it manually" on-ramp, which is where confabulation starts. The reviewer re-reads state from disk on every call, which is also what makes it a drift-watchdog.
