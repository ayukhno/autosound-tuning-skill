...
- **A clarification after the method** (`r-L_17 (sw) noXO`, `sw (imp) case35l, NO cotton wool`) is free-form text about how the graph was taken, and it is welcome. **It is the same measurement**: `r-L_17 (sw) noXO` answers a round that asked for `r-L_17 (sw)` (the user, 2026-09-16). The tools keep the text (`params`) and never count it as identity. That is the difference from a **modifier**, which sits before `_N` (`r-L FX_17 (sw)`) and does make another measurement. A position among the clarification (`x0`, `p3`) is still a position, and a position is identity (skill #34).
