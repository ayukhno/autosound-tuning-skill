...
- **Transient experiment tags** — while A/B-testing a candidate change, tag the variant in the name (`i`/`INV` = inverted polarity, `+Δτ` = an added delay trial, etc.) so the two readings don't get confused. The tag is **temporary**: once the change is **baked into the base** it drops from the name (the measurement is just `_N+1` at the new config). **the ledger HEAD (`state/<preset>/v_NNN.json`) is the source of truth for what's in the base** — `dsp-state-current` is its generated view — the name tags only the experiment in flight, not the committed state.
...
- **Multiple active slots → one machine-checked pointer.** When the DSP holds several presets in physical slots (Helix Slot 1/2/3), which one is *loaded right now* is not a prose note — it's `rew_tool/state/registry.json` (`state.py registry set-active <preset>`). `state.py --root <project>/state registry render` generates the multi-slot `dsp-state-current` view (a loud active-slot banner + one isolated row per slot), and the apply gate refuses a change aimed at any non-active slot. This kills the cross-slot anchoring trap (computing filters off a neighbour slot's gains — issue #5).
