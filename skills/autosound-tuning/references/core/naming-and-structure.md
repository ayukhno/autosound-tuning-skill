...
This is the practical face of "the foundation is curve-agnostic" (`preset-strategy.md`): hardware defines the instrument, the curve only voices it.
...
- **A fact CARRIED IN from another project says so, and is not owed a second measurement.** An export writes the value and the moment it was measured, and both are right; what it cannot write is that THIS build did not establish it — seven `channels[].fs_hz` facts arrived carrying `source: "measured"` with the source build's own timestamp and landed in a project created days later on another machine, indistinguishable from measurements taken there (S-024). Mark them: `python3 rew_tool/project.py <dir> mark-imported channels.tw-L.fs_hz --from <the other project>`. The value stands, `at` keeps the moment it was measured THERE, `source` keeps saying `measured` — because it was — and only the origin changes. **The Arbiter's rule, 2026-09-19:** «імпеданс складна штука і міряти його другий раз це подвиг» — his word closes it, a protective high-pass may be derived from an imported Fs, and nothing gates on it. It stays marked, and every report that uses it names where it came from (`contract.py check` lists the carried-in facts, and says when their source project is gone).

## 3. Measurement naming (REW)

Measurement name = **`<channel|pair|combo|joint>[ <modifier>]_<N> (<method>)[ <clarification>]`** — or, for an impedance sweep only, **`<channel> (imp)[ <clarification>]`** with no `_N`. This section is the grammar's one home in prose; `rew_tool/naming.py` is its one reader in code, and `naming.py <project> parse "<title>"` says why a title is refused.

- **Channel / pair / combo / joint codes** → `autosound_context.md §5` for the human-readable agreement, `glossary.json` (or `project.json`'s `glossary` key) for the machine copy `rew_tool/naming.py` actually reads (this car: `sw`, `w-L/R`, `m-L/R`, `tw-L/R`, `r-L/R`, `c-H/c-L`; pairs `Ws/Ms/TWs`; combos `ALL`, `ALL+C`; joints `SW+Ws`, `L w+m`, `R m+tw`; full side `L`/`R`). The grammar itself is code, not just this prose — `naming.py generate_name/parse_name/expected_series/validate_series`, so a measurement checklist is derived (phase × glossary × `_N`) instead of hand-typed per project.
- **One notation, everywhere (the Arbiter, 2026-09-22: «правильна назва через "-"»):** a hyphen carries the side or the variant (`w-L`, `sw-f`, `c-H`); `_` appears ONLY before the series number (`_49`). `naming.py parse` refuses a code with `_` in it and names the hyphen form. A channel's `id` in `project.json` follows the same rule: when `contract.py check` reports ids outside it, **ask the user and offer to put it right in the session** (`project.py <project> fix-ids` shows the change, `--apply` writes it on his OK) (S-042).
- **Method suffix:** `(rta)` = MMM RTA (for magnitude/tone) · `(sw)` = acoustic sweep (for phase/timing/distortion) · `(imp)` = impedance sweep (a driver's resonance in the install, for the protective high-pass — skill #33). `(imp)` needs no `_N`: an impedance measurement is not tied to a DSP state.
- **A clarification after the method** (`r-L_17 (sw) noXO`, `sw (imp) case35l, NO cotton wool`) is free-form text that says what the measurement is and what it is for, and it is welcome. **It makes another measurement in the same series**: `r-L_17 (sw) noXO` is not `r-L_17 (sw)`, and both are `_17` (the user, 2026-09-16). The tools keep the text (`params`) and count it as a modifier typed in another place, so `r-L noXO_17 (sw)` is the same measurement. A position among it (`x0`, `p3`) is still a position (skill #34).
- **The CANONICAL title is what `naming.py name <code> <version_n> <method>` emits** — one digit for the series (`sw_1 (sw)`), never zero-padded. A title REW holds in another spelling is the same measurement — the comparison runs on `naming.name_key`, so `sw_01 (sw)` and `sw_1 (sw)` are one — but the check now SAYS the two differ (`ok*`, with the rename), because a front-end finds a measurement by its literal REW title and cannot follow a normalisation it was never told about: on a live session `naming.py check` said `ok` on all 16 while the front-end said `0 usable, 16 missing`, over the same REW session (skill `#47`). **The fix is a RENAME, never a re-measurement** — `rew_api.rename_measurement`, and REW's `uuid` survives it (one measurement carried three titles in one session and kept its id). `validate_series` returns `renames` as `{title on disk: canonical title}`, which is the "new name" column a read-from-REW step fills in.
- **Titles are matched as typed; a mismatch is a question.** Capturing is manual, and so is naming: (a) the round needs 7 captures — they are taken in the planned order under one name, and the session renames them to the planned titles (`rew_api.rename_measurement`); (b) 10 are taken and named by hand — the planned ones are found by name, and where a name does not match the session **asks** (in TCC the owner maps what is what by hand); the extra ones say by their own names what they are and what for; (c) captures with no round at all are mapped by hand — an exception for testing, not a route. No tool guesses which capture was meant (the user, 2026-09-16).
- **Modifiers:** `FX` = measured with Helix FX on; `c` / `c FX` = front without / with FX.
- **Two subwoofers (2026-08-24, additive):** one sub stays `sw`. With a front and a rear sub the channels are **`sw-f`** and **`sw-r`**, their pair is **`SWs`** (like `Ws`), and the joint to the woofers is **`SWs+Ws`**. All of it is glossary data (`pairs`, `joints`) — no old title changes, `sw_01 (rta)` stays what it is. The tools read two subs as ONE member at the junction (`SWs`, their sum) and their mutual alignment as a PAIR, never as a crossover between them (`predict.py`, `analyze-joints --from-state`); `target_bands` gives each of the two `is_stereo: true` so their summation offset applies.
...
- **`_N` = the number of the measurement series** (the word for it in a report: «серія», §1a), **and it is scoped to ONE project**. Two projects both have a `_49` and they mean different DSP states on different days, so a series taken elsewhere does not keep its number here: a round refuses a number that is not this project's own — already used, or the next after its highest — unless its ORIGIN is on record (`capture-start <N> --origin <project>:<their _N>`). Bought when a session said out loud that a set was «серія `_49` зі старого проєкту» and then opened the round as series 49 anyway, writing another build's numbering into this project's record (S-048). The acoustic question — was anything changed in the install between the two dates — stays the Arbiter's; this one is mechanical and should never have reached him. Every capture of one series shares it — `m-L_04 (sw)` and, in the same series, `m-L_04 (rta)`. **The relation runs one way: a DSP state has several series** — changing the DSP before the next measurement starts a new series (three changes measured one after another are `_06`, `_07`, `_08`), and one state can be measured in more than one series. **A measurement need not be tied to the DSP at all:** an impedance sweep `(imp)` has no `_N` (the user, 2026-09-17). **Saving is a separate event** — to a DSP slot or a backup file, at any `N` or none — and neither moves `_N` nor defines it (hub TCC-016). This is what lets the changelog line up "before vs after a change" (`ALL_17` vs `ALL_18` shows what an EQ import moved). A sweep or an RTA with no `_N` is refused. **`_N` is not the ledger's `v_NNN`** — see §5: the two are different counters, and a ledger version given for `_N` is refused rather than turned into titles no REW list will ever hold (skill #37).
...
- The per-measurement `_N` lives *inside* the file, so the same day can hold several series. Don't encode `_N` in the filename — the date groups, `_N` numbers the series.
...
- **Curve library in this REW** (each is a candidate session target — naming any of these is a strong "this is a tuning task" signal): `Flat`, `Audiofrog`, `Whitledge`, `Half Whitledge`, `Harman`, `JBL`, `JL Audio`, `Jazzi`, `Jazzi v2`, `RAW-Cat`, `RAW-Cat (uni)`, `ResoNix Accurate`, `ResoNix Laid-Back`, `ATF`, `ATF EQ (Helix)`, `EPY`, `Hanatsu`, `Arkadij`, `Arkadij v2`. Some are industry curves (Harman, Audiofrog, ResoNix, JL Audio, JBL), some custom. The active one is whatever the session picks; the list grows. **The intake's list follows NTT's presets, not this REW** (2026-09-22, `intake.NTT_CURVE_PRESETS`): Audiofrog, Whitledge, Half Whitledge, Harman, JBL, JL Audio, Jazzi, Jazzi v2, RAW-Cat, ResoNix Accurate, ResoNix Laid-Back, ResoNix 2026, ATF Daily, ATF SQ, EPY, Hanatsu, plus the skill's own bundled `SQ-Comp-Ref` and «own / other». The names above that NTT does not list — `Flat`, `RAW-Cat (uni)`, `ATF`, `ATF EQ (Helix)`, `Arkadij`, `Arkadij v2` — are this REW's and this project's, and stay valid session targets; an intake records one of them as «own / other».
...
> **Where each layer is canonical:** this car's channel glossary → `autosound_context.md §5`. The step path → `process-phases.md`. Current applied config (vN) → the ledger HEAD; `dsp-state-current` renders it. Active curve → `target-curve-*`. This file holds only the reusable *rules* tying them together.
