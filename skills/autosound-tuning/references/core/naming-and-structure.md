...
- **Method suffix:** `(rta)` = MMM RTA (for magnitude/tone) · `(sw)` = acoustic sweep (for phase/timing/distortion) · `(imp)` = impedance sweep (a driver's resonance in the install, for the protective high-pass — skill #33). `(imp)` needs no `_N`: no DSP state moves a resonance.
...
- **Transient experiment tags** — while A/B-testing a candidate change, tag the variant in the name (`i`/`INV` = inverted polarity, `+Δτ` = an added delay trial, etc.) so the two readings don't get confused. The tag is **temporary**: once the change is **baked into the base** it drops from the name (the measurement is just `_N+1`, the new DSP state). **the ledger HEAD (`state/<preset>/v_NNN.json`) is the source of truth for what's in the base** — `dsp-state-current` is its generated view — the name tags only the experiment in flight, not the committed state.
...
- The per-measurement `_N` lives *inside* the file, so the same day can hold several DSP states. Don't encode `_N` in the filename — the date groups, `_N` numbers the states.
