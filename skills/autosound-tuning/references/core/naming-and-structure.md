...
Measurement name = **`<channel|pair|combo|joint>[ <modifier>]_<N> (<method>)[ <free-form parameters>]`** — or, for an impedance sweep only, **`<channel> (imp)[ <free-form parameters>]`** with no `_N`. This section is the grammar's one home in prose; `rew_tool/naming.py` is its one reader in code, and `naming.py <project> parse "<title>"` says why a title is refused.
...
- **Free-form parameters after the method** (`r-L_17 (sw) noXO`, `sw (imp) case35l, NO cotton wool`) are how titles get typed in the car, and they are welcome. They read as a modifier typed in another place — part of WHICH measurement it is, so `r-L_17 (sw) noXO` is not the solo `r-L_17 (sw)` (a sweep without the crossover is not the sweep with it) and is the same measurement as `r-L noXO_17 (sw)`. A position among them (`x0`, `p3`) is still a position (skill #34).
