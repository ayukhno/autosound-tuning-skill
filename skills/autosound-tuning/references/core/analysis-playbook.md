...
| Phase rotation / excess GD at joints | Group delay — at a junction, the energy arrival τ through FDW-6 (the direct sound); REW's GD is the cabin's, labelled by its window | Sweep | `windows.windowed_group_delay` on the IR (`get_impulse_response`); `get_group_delay` for the cabin |
...
- **Most of a measured cabin group delay moves with the microphone — read junction timing through the direct sound.** Seven hand-held positions around one seat (`_49`, junctions 1.6 and 2.7 kHz, both sides): per driver, the standard deviation across positions of REW's own group delay (1/24 octave, REW's window), median over fc/2…2·fc, was 1.5–2.9 ms; of the energy arrival τ through FDW-8 (`windows.windowed_group_delay`, 1/12 octave) 0.3–0.7 ms — ×0.16–0.29 of REW's in all eight driver cases, and FDW-6 read no worse than FDW-8 in the same run (hub `RES-010`). So junction timing is τ through FDW, REW's GD is the whole-cabin reading, each number names its window, and the two are never subtracted as one quantity. Validated 800–5400 Hz; lower down a 6–8-cycle window grows long and takes the cabin back in.
