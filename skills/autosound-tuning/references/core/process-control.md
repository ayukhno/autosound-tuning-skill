...
|---|---|---|---|
| **A** | **Claude drives (Generator) + Gemini as Advisor** — **ONE reviewer call per round** at solution-search nodes: crossover strategy (Phase 1), the round's EQ plan (Phase 2), the Phase-3 verdict, staging/imaging decisions. Package the whole batch, not per-parameter calls | Highest | Needs both AIs; one extra call per round — buys anti-anchoring + a drift-watchdog |
