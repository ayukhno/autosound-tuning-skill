...
- **`process/`** — `python rew_tool/state/process.py <project>/process enter-phase -1` at the start of this very phase, then `add-step`/`done` for its own checklist items as you clear them (§0.5's gates are natural step boundaries). This is what makes THIS phase itself resumable, not only the ones after it.
- **Verify the whole set before moving on:** `python rew_tool/contract.py check <project>` — should report every machine file present and valid before the Phase-0 gate clears.
