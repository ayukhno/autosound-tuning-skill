...
4. **Gain staging:** minimum amp gain + maximum DSP level = better SNR; gain up to the first THD jump → back off ~10% (the RTA/THD procedure is in `helix-vcp-workflow.md`, the principle is generic for any DSP). **These gains are set before a house curve exists.** A curve with a large bass shelf changes what "correct" means for the sub amplifier, and Phase 1's levels say so (`virtual-first.md` 1.6, `level_offsets.py`): a cut-only spread wider than 3 dB is a gain-structure finding, with the amp change worked out (#51).
