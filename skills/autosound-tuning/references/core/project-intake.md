...
4. **Gain staging:** minimum amp gain + maximum DSP level = better SNR; gain up to the first THD jump → back off ~10% (the RTA/THD procedure is in `helix-vcp-workflow.md`, the principle is generic for any DSP). **These gains are set before the tune, and the tune does not move them.** Levels live in the DSP (the Arbiter, 2026-09-23): a curve with a large bass shelf lifts the sub in the DSP, within its channel gain range (`virtual-first.md` 1.6, `level_offsets.py`, #51). An amp gain moves only when that range runs out; it is the user's decision, and he tells the session what he turned (`process.py <dir> amp-gain sw=+3`), so the next series is compared with the known change taken out.
