...
5. **Naming + glossary — AGREE BEFORE ANY MEASUREMENT.** ⛔ **Gate:** don't measure until the channel codes (`sw / w-L/R / m-L/R / tw-L/R / c / r`; two subwoofers → `sw-f` + `sw-r`, pair `SWs`, joint `SWs+Ws` — `naming-and-structure.md`) AND the convention `<ch>_<vN> (sw|rta)` are set with the user (`naming-and-structure.md §3`). The recurring slip is running off to measure with un-agreed names → an unusable history. **Write it, don't just agree it** — `glossary.json`/`project.json` (§5), the machine copy `naming.py` and a measurement checklist actually read.
...
- **The first ledger snapshot** (`rew_tool/state/`) — seed it with **every tier the DSP profile declares populated**, not just physical outputs: a Helix project's first `v_001` needs both `channels` AND `virtual_channels` rows from the start (empty/placeholder is fine for a slot with no driver yet — mark it `"hidden": true`, SCR-003 — but the tier key itself must exist, since `apply.propose` can only address a tier that's already there). **Multiple presets in physical DSP slots** → also set the live one (`state.py registry set-active <preset>`); `registry render` generates the multi-slot `dsp-state-current` (active-slot banner + isolated per-slot rows) so the model can't anchor on the wrong slot (issue #5).
- **`process/`** — **`enter_phase("-1")` if your front-end offers the tool, otherwise `python3 rew_tool/state/process.py <project>/process enter-phase -1`** — and it goes FIRST, before the first question, not somewhere in this list. A model that decided to ask its questions first dropped this without ever disobeying a sentence, and the phase went unrecorded for the whole session. Then `add-step`/`done` for its own checklist items as you clear them (§0.5's gates are natural step boundaries). This is what makes THIS phase itself resumable, not only the ones after it.
- **Verify the whole set before moving on:** `python3 rew_tool/contract.py check <project>` — should report every machine file present and valid before the Phase-0 gate clears.
