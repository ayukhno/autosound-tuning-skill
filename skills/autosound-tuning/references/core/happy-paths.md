...
3. **Review + bank.** ONE critic pass on the round's package → `apply.propose(…, evidence=[the desk result file, or the titles], reviewed=<that review>)` — the desk's own computation is the evidence in the early phases, and the car is visited to VERIFY several changes at once (a structural move without them banks as a 🟠 candidate, not in the slot) → Arbiter enters the sheet → `attest` → the re-measure that `state.py … verification-set <preset> <va> <vb> --series N` derives from what moved, not the whole series.
