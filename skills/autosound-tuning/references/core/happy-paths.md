...
3. **Review + bank.** ONE critic pass on the round's package → `apply.propose(…, evidence=[the titles or the verdict file], reviewed=<that review>)` (a structural move without them banks as a 🟠 candidate, not in the slot) → Arbiter enters the sheet → `attest` → the re-measure that `state.py … verification-set <preset> <va> <vb> --series N` derives from what moved, not the whole series.
