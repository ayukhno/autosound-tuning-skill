...
| `eq_gate.min_phase_verdict` | `verdict` | the data does not reach both edges of the window, or < 8 points in it; UNSTABLE over repeats that disagree or move more than 10°; PROVISIONAL without repeats; corroborating only above 200 Hz | a narrower window, said so; the gate's `check`; the ellipsoid's stays/moves above Schroeder |
...
| joint delay / polarity / APF (`analyze-joints`) | solos on the loopback base whose protective chain is recorded on the capture round, or working captures; a measured pair of the joint's two channels | a **baseline** solo nobody marked — `check`, no number; a joint with no measured pair — no verdict, the pair to measure named (`--no-pair`: a reading marked NOT BANKABLE) | record the round first (`capture-protective`); a phase read through an unrecorded `LR4 @100` is out by tens of degrees and looks fine (`project-intake.md §3`) |
| `predict --align` (delay × polarity by sum loss) | junctions whose members' arrival difference the search window reaches | a score one cycle off — an ALIAS — is said by name, never taken silently; the physical candidate goes into the proposal | check the impulse before entering a delay the note calls off the arrival alignment |
| `ellipsoid` (stays / moves, σ(f), the Q ceiling) | ≥ 2 distinct positions, better 6 + 3 centre returns | one position (no spread), one centre sweep (no floor); a Q ceiling with no staying feature is BORROWED and says so | capture the ellipsoid before the EQ packages, or accept unverified-in-space by name |
| `eq_propose` resonance packages | a peak of medium width, staying, minimum-phase, away from a junction | a dip (not without `--allow-boost` AND the gate), within ±1 oct of a junction (the delay's business), no ellipsoid (unverified in space, said), the phase gate out of its band (said) | the junctions are settled first (1.3); the ellipsoid is what turns "unverified" into a verdict |
