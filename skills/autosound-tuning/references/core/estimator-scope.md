...
| joint delay / polarity / APF (`analyze-joints`) | solos on the loopback base whose protective chain is recorded on the capture round, or working captures | a **baseline** solo nobody marked — `check`, no number | record the round first (`capture-protective`); a phase read through an unrecorded `LR4 @100` is out by tens of degrees and looks fine (`project-intake.md §3`) |
