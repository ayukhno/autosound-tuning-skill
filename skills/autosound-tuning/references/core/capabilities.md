...
| has this CABIN been described before / чи є вже щось про цей кузов | the library's entry for EXACTLY this body, or `None` — which is the answer that says "interview from scratch"; and, separately, how many measured flaw rows a previous project on the same body would hand over | `car_profile.py --find "<make>" "<model>" "<body>"`, `--list`; `project_seed.py --describe <dir>` for the row count | exact body only — a platform sibling is never matched and never NAMED, so there is no "did you mean" | −1.2 | field | `core/project-intake.md` §4 |
