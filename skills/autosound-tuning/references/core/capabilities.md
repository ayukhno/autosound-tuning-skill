...
| can the flaw map be READ by the owner / чи можна віддати мапу власникові | which owner-facing rows still owe the owner's own sentence (a machine `DRAFT:` does not count); exits non-zero while any does | `contract.py check <dir> --phase0-gate` | the map written · refuses nothing — an empty map owes nothing | 0 → 1 | selftest | `phases/phase_0_baseline.md` |
