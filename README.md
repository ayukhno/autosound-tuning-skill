...
```sh
curl -fsSL https://raw.githubusercontent.com/ayukhno/autosound-tuning-skill/v3.0.60/install.sh | bash -s -- --github
```
On Windows they are `-WithOmp`, `-GitHub`, `-Terminal`, `-DryRun`, on this form:
```powershell
& ([scriptblock]::Create((irm https://raw.githubusercontent.com/ayukhno/autosound-tuning-skill/v3.0.60/install.ps1))) -GitHub
```
