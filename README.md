...
* **Which model to trust as driver, so far:** **Claude Opus** has given the most consistently stable results. **Sonnet 5** works but still comes across as less sure of itself in this role — worth double-checking its calls for now. **Fable 5** produced the best results of any model so far, though that was on the *meta* job of building and auditing the skill itself (see [audit-fable-2026-07-11.md](audit-fable-2026-07-11.md)), not on driving a tune — still promising if you're extending the skill rather than running one. **Gemini** lost some capability as the process rules grew more complex; an audit and optimization pass has since simplified them, which should help, but that hasn't been verified in a real session yet — feedback from the community is welcome here.
